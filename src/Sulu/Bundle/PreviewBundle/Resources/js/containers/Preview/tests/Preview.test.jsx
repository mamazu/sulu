...
const Router_1 = __importStar(require("sulu-admin-bundle/services/Router"));
...
jest.mock('sulu-admin-bundle/services/Requester', () => ({
    get: jest.fn().mockImplementation((route) => new Promise((resolve) => {
        if (route === '/start') {
...
jest.mock('sulu-admin-bundle/containers/Form/stores/ResourceFormStore', () => jest.fn((resourceStore) => {
    var _a;
    return {
        resourceKey: resourceStore.resourceKey,
        locale: (_a = resourceStore.observableOptions) === null || _a === void 0 ? void 0 : _a.locale,
    };
}));
jest.mock('sulu-admin-bundle/services/ResourceRequester', () => ({
    getList: jest.fn(),
}));
jest.mock('sulu-admin-bundle/stores/ResourceStore', () => jest.fn((resourceKey, id, observableOptions) => {
    return {
        resourceKey,
        observableOptions,
    };
...
beforeEach(() => {
    jest.resetModules();
    Preview_1.default.mode = 'on_request';
    Preview_1.default.audienceTargeting = false;
    stores_1.webspaceStore.getWebspace.mockReturnValue({ segments: [] });
});
test('Render correct preview', () => {
    const resourceStore = new ResourceStore_1.default('pages', 1);
    const formStore = new ResourceFormStore_1.default(resourceStore, 'pages');
    const router = new Router_1.default({});
    stores_1.webspaceStore.getWebspace.mockReturnValue({
        segments: [
            { key: 's', name: 'Summer', default: false },
            { key: 'w', name: 'Winter', default: true },
        ],
    });
    const preview = (0, enzyme_1.mount)(<Preview_1.default formStore={formStore} router={router}/>);
...
test('Render correct preview with target groups', () => {
    ...
    ResourceRequester_1.default.getList.mockReturnValue(targetGroupsPromise);
    const resourceStore = new ResourceStore_1.default('pages', 1);
    const formStore = new ResourceFormStore_1.default(resourceStore, 'pages');
    const router = new Router_1.default({});
    Preview_1.default.audienceTargeting = true;
    const preview = (0, enzyme_1.mount)(<Preview_1.default formStore={formStore} router={router}/>);
    const startPromise = Promise.resolve();
    const previewStore = preview.instance().previewStore;
    previewStore.start.mockReturnValue(startPromise);
...
test('Render nothing if separate window is opened and rerender if it is closed', () => {
    ...
    const formStore = new ResourceFormStore_1.default(resourceStore, 'pages');
    const router = new Router_1.default({});
    const preview = (0, enzyme_1.shallow)(<Preview_1.default formStore={formStore} router={router}/>);
    const startPromise = Promise.resolve();
    const previewStore = preview.instance().previewStore;
    previewStore.start.mockReturnValue(startPromise);
    previewStore.starting = false;
...
test('Change webspace in PreviewStore when selection of webspace has changed', () => {
    ...
    const resourceStore = new ResourceStore_1.default('pages', 1, { locale });
    const formStore = new ResourceFormStore_1.default(resourceStore, 'pages');
    const router = new Router_1.default({});
    const preview = (0, enzyme_1.shallow)(<Preview_1.default formStore={formStore} router={router}/>);
    const startPromise = Promise.resolve();
    ...
    preview.instance().handleStartClick();
    return startPromise.then(() => {
        expect(PreviewStore_1.default).toBeCalledWith('pages', undefined, locale, 'sulu_io', undefined);
...
test('Use router attribute to determine webspace', () => {
    const locale = mobx_1.observable.box('ru');
    const resourceStore = new ResourceStore_1.default('pages', 1, { locale });
    const formStore = new ResourceFormStore_1.default(resourceStore, 'pages');
    const router = new Router_1.default({});
    ...
    preview.instance().handleStartClick();
    return startPromise.then(() => {
        expect(PreviewStore_1.default).toBeCalledWith('pages', undefined, locale, 'example', undefined);
    });
});
test('Change segment in PreviewStore when selection of segment has changed', () => {
    ...
    preview.instance().handleStartClick();
    return startPromise.then(() => {
        expect(PreviewStore_1.default).toBeCalledWith('pages', undefined, undefined, 'sulu_io', 'w');
...
test('React and update preview when data is changed', () => {
    const resourceStore = new ResourceStore_1.default('pages', 1);
    const formStore = new ResourceFormStore_1.default(resourceStore, 'pages');
    formStore.data = mobx_1.observable.map({ title: 'Test' });
    formStore.loading = false;
    formStore.type = mobx_1.observable.box('default');
    const router = new Router_1.default({});
    const preview = (0, enzyme_1.mount)(<Preview_1.default formStore={formStore} router={router}/>);
    const startPromise = Promise.resolve();
...
test('React and update-context when schema is changed', () => {
    ...
    formStore.loading = false;
    formStore.type = mobx_1.observable.box('default');
    formStore.schema = mobx_1.observable.box({ title: { label: 'Title' } });
    const router = new Router_1.default({});
    const preview = (0, enzyme_1.mount)(<Preview_1.default formStore={formStore} router={router}/>);
    const startPromise = Promise.resolve();
...
test('React and restart when locale is changed', () => {
    ...
    formStore.type = mobx_1.observable.box('default');
    formStore.schema = mobx_1.observable.box({ title: { label: 'Title' } });
    formStore.locale = mobx_1.observable.box('en');
    const router = new Router_1.default({});
    const preview = (0, enzyme_1.mount)(<Preview_1.default formStore={formStore} router={router}/>);
    const startPromise = Promise.resolve();
    const updateContextPromise = Promise.resolve('<h1>Sulu is awesome</h1>');
...
test('Change dateTime in PreviewStore when DatePicker changed', () => {
    ...
    return startPromise.then(() => {
        preview.update();
        expect(PreviewStore_1.default).toBeCalledWith('pages', undefined, undefined, 'sulu_io', undefined);
        const date = new Date();
