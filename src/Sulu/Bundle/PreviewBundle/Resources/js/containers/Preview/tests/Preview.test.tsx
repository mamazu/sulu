...
jest.mock('debounce', () => jest.fn((value: any) => value));

jest.mock('../stores/PreviewStore', () => jest.fn(function(resourceKey: any) {
...
jest.mock('sulu-admin-bundle/services/Requester', () => ({
    get: jest.fn().mockImplementation((route: string) => new Promise((resolve: any) => {
        if (route === '/start') {
...
jest.mock('sulu-admin-bundle/stores/ResourceStore', () => jest.fn(
    (resourceKey: any, id: any, observableOptions: any) => {
        return {
            resourceKey,
            observableOptions,
        };
...
jest.mock('sulu-admin-bundle/services/Router/Router', () => jest.fn(function(history: any) {
...
test('Render nothing if separate window is opened and rerender if it is closed', () => {
    const previewWindow = {addEventListener: jest.fn()} as const;
