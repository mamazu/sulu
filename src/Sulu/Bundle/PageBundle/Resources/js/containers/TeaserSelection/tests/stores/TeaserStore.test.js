...
test('Load and reload teasers when new ones are added', () => {
    ...
    services_1.ResourceRequester.getList.mockReturnValue(teaserPromise1);
    const teaserStore = new TeaserStore_1.default(mobx_1.observable.box('de'));
    expect(services_1.ResourceRequester.getList).toHaveBeenLastCalledWith('teasers', { ids: [], locale: 'de' });
    return teaserPromise1.then(() => {
        ...
        services_1.ResourceRequester.getList.mockReturnValue(teaserPromise2);
        teaserStore.add('pages', 1);
        teaserStore.add('pages', 2);
        teaserStore.add('contacts', 1);
        expect(services_1.ResourceRequester.getList)
...
test('Add items only once to teaser store', () => {
    const teaserStore = new TeaserStore_1.default(mobx_1.observable.box('en'));
    teaserStore.add('pages', 1);
    teaserStore.add('pages', 1);
...
test('Use findById function to load teasers', () => {
    ...
    services_1.ResourceRequester.getList.mockReturnValue(teaserPromise);
    const teaserStore = new TeaserStore_1.default(mobx_1.observable.box('en'));
    expect(services_1.ResourceRequester.getList).toHaveBeenLastCalledWith('teasers', { ids: [], locale: 'en' });
