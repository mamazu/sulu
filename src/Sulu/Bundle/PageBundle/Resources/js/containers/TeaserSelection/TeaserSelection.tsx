...
type Props = {
    ...
    onChange: (arg1: TeaserSelectionValue) => void,
    onItemClick?: (id: number | string, item?: TeaserItem | null | undefined) => void,
...
class TeaserSelection extends React.Component<Props> {
    ...
    @observable
    openedOverlay: string | null | undefined = undefined;
    ...
    @computed
    get teaserItems(): Array<TeaserItem> {
        return this.props.value.items.map((teaserItem) => ({
            ...this.teaserStore.findById(teaserItem.type, teaserItem.id),
            ...(Object.keys(teaserItem).reduce<Record<string, any>>((clearedTeaserItem, key) => {
    ...
    @computed
    get presentationOptions(): Array<PresentationItem> | null | undefined {
    ...
    @action handleApply = (item: TeaserItem) => {
        ...
        const value = {...this.props.value} as const;
    ...
    @action handleConfirm = (items: Array<any>) => {
    ...
    @action handleAddClick = (provider?: string | null) => {
        this.openedOverlay = provider;
    };

    @action handlePresentationClick = (presentation?: string | null) => {
        const {onChange, value} = this.props;
