...
jest.mock('sulu-admin-bundle/containers', () => {
    ...
    return ({
        ...
        DefaultLoadingStrategy: require('sulu-admin-bundle/containers/List/loadingStrategies/DefaultLoadingStrategy').default,
        ...
        ListStore: (_a = class {
            ...
            __setFunctionName(_a, "ListStore"),
            _a.getActiveSetting = jest.fn(),
            _a),
        formMetadataStore: {
...
test('Render PageList', () => {
    ...
    const webspaceKey = mobx_1.observable.box('sulu');
    const webspace = Object.assign(Object.assign({}, TestHelper_1.defaultWebspace), { localizations: undefined });
    const PageList = require('../PageList').default;
    const router = new services_1.Router({});
    router.attributes = {
        webspace: 'sulu',
    };
    const webspaceOverview = (0, enzyme_1.mount)(<PageList route={router.route} router={router} 
    // $FlowFixMe
    webspace={webspace} webspaceKey={webspaceKey}/>);
...
test('Should show the locales from the webspace configuration for the toolbar', () => {
    ...
    const PageList = require('../PageList').default;
    const toolbarFunction = (0, TestHelper_1.findWithHighOrderFunction)(withToolbar, PageList);
    const webspaceKey = mobx_1.observable.box('sulu');
    const webspace = Object.assign(Object.assign({}, TestHelper_1.defaultWebspace), { localizations: undefined, key: 'sulu', allLocalizations: [{ localization: 'en', name: 'en' }, { localization: 'de', name: 'de' }] });
    const router = new services_1.Router({});
    router.attributes = {
        webspace: 'sulu',
    };
    ...
    expect(toolbarConfig.locale.options).toEqual(expect.arrayContaining([
...
test('Should change excludeGhostsAndShadows when value of toggler is changed', () => {
    ...
    const toolbarFunction = (0, TestHelper_1.findWithHighOrderFunction)(withToolbar, PageList);
    const webspaceKey = mobx_1.observable.box('sulu');
    const webspace = Object.assign(Object.assign({}, TestHelper_1.defaultWebspace), { localizations: undefined, allLocalizations: [{ localization: 'en', name: 'en' }, { localization: 'de', name: 'de' }], key: 'sulu' });
    const router = new services_1.Router({});
    router.attributes = {
        webspace: 'sulu',
    };
...
test('Should use CacheClearToolbarAction for cache clearing', () => {
    ...
    const pageList = (0, enzyme_1.mount)(<PageList route={router.route} router={router} 
    // $FlowFixMe
    webspace={webspace} webspaceKey={webspaceKey}/>);
