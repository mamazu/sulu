...
test('Render webspace select with children when webspaces are not loaded yet', () => {
    ...
    const webspace = {key: 'sulu_blog', localizations: [{locale: 'en', default: false}, {locale: 'de', default: true}]} as const;

    webspaceStore.getWebspace.mockImplementation((key: any) => {
        if (key === 'sulu_blog') {
            return webspace;
        }
    });
    ...
    const webspaceTabs = mount(
        <WebspaceTabs isRootView={true} route={route} router={router}>
            {(props: any) => <h1>{props && props.webspace && props.webspace.key}</h1>}
        </WebspaceTabs>
    );
...
test('Load webspace userStore if no route attribute is given', () => {
    userStore.getPersistentSetting.mockImplementation((key: any) => {
...
test('Save and update webspace when select value is changed', () => {
    ...
    const webspace1 = {key: 'sulu', localizations: [{locale: 'en', default: true}]} as const;
    const webspace2 = {
        key: 'sulu_blog',
