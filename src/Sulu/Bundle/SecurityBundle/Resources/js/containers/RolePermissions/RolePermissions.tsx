...
type Props = {
    ...
    permissionCheck?: boolean | null | undefined,
    resourceKey: string,
...
class RolePermissions extends React.Component<Props> {
    ...
    handleChange = (newSystemValue: RolePermissionsType, system: string) => {
        ...
        onChange({
            ...Object.keys(value).reduce<Record<string, any>>((values, roleId) => {
    ...
    render() {
        ...
        return systems.reduce<Array<any>>((systemMatrices, system) => {
            ...
            const systemValues = Object.keys(value).reduce<Record<string, any>>((systemValues, roleId) => {
                if (!systemRoles.some((systemRole) => systemRole.id.toString() == roleId)) {
                    return systemValues;
                }
