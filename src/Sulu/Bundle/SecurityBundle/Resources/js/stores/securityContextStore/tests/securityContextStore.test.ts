...
test('Load available actions for permissions with given keys', () => {
    ...
    return securityContextStore.loadAvailableActions('test').then((actions: any) => {
...
test('Load security contexts for entire system', () => {
    ...
    return securityContextStore.loadSecurityContextGroups('Sulu').then((securityContexts: any) => {
        expect(log.warn).toBeCalled();
