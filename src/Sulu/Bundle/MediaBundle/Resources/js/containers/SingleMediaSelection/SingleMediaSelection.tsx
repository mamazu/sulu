...
type Props = {
    ...
    onChange: (selectedId: Value, media?: Media | null | undefined) => void,
    onItemClick?: (itemId?: number | null | undefined, value?: Media | null | undefined) => void,
    types: Array<string>,
    valid: boolean,
    value: Value
};
...
class SingleMediaSelection extends React.Component<Props> {
    ...
    handleOverlayConfirm = (selectedMedia: any) => {
        this.singleMediaSelectionStore.set(selectedMedia);
        this.closeOverlay();
    };

    handleDisplayOptionClick = (displayOption?: DisplayOption | null) => {
        const {onChange, value} = this.props;

        onChange({...value, displayOption});
    };

    handleItemClick = (itemId?: number | null, item?: Media | null) => {
