...
test('Closing the overlay should call the onClose callback', () => {
    ...
    const cropOverlay = (0, enzyme_1.shallow)(<CropOverlay_1.default id={4} image="test.jpg" locale="de" onClose={closeSpy} onConfirm={jest.fn()} open={true}/>);
    cropOverlay.find('Overlay').prop('onClose')();
    expect(MediaFormatStore_1.default).toBeCalledWith(4, 'de');
...
test('Reset format croppings when closing overlay', () => {
    ...
    const cropOverlay = (0, enzyme_1.shallow)(<CropOverlay_1.default id={7} image="test.jpg" locale="en" onClose={jest.fn()} onConfirm={jest.fn()} open={true}/>);
    const cropData = {
        'test2': {
            cropHeight: 30,
            cropWidth: 60,
    ...
    return formatsPromise.then(() => {
        ...
        cropOverlay.find('withContainerSize(ImageRectangleSelection)').prop('onChange')({ height: 60, left: 200, top: 20, width: 20 });
...
test('Save changes of formats', () => {
    ...
    const formatsPromise = Promise.resolve(formats);
    formatStore_1.default.loadFormats.mockReturnValue(formatsPromise);
    const cropOverlay = (0, enzyme_1.shallow)(<CropOverlay_1.default id={7} image="test.jpg" locale="en" onClose={jest.fn()} onConfirm={confirmSpy} open={true}/>);
    const cropData = {
        'test1': {
            cropHeight: 30,
            cropWidth: 60,
    ...
    return formatsPromise.then(() => {
        ...
        cropOverlay.find('withContainerSize(ImageRectangleSelection)').prop('onChange')({ height: 120, left: 100, top: 70, width: 30 });
