...
const MediaUploadStore_1 = __importDefault(require("../MediaUploadStore"));
...
test('Calling the "update" method should make a "POST" request to the media update api', () => {
    services_1.resourceRouteRegistry.getUrl.mockReturnValue('/media/1?action=new-version&locale=en');
    ...
    expect(services_1.resourceRouteRegistry.getUrl).toBeCalledWith('detail', 'media', { action: 'new-version', id: 1, locale: 'en' });
...
test('Promise returned by "update" method should be resolved if request is successful', () => {
    ...
    return updatePromise.then(() => {
        ...
        expect(mediaUploadStore.media).toEqual({ id: 1, locale: 'en', mimeType: 'image/jpeg', title: 'updated-title', thumbnails: {}, url: '', adminUrl: '' });
...
test('Promise returned by "update" method should be rejected if request has error status', (done) => {
    ...
    setTimeout(() => {
        expect(mediaUploadStore.uploading).toEqual(false);
        expect(mediaUploadStore.progress).toEqual(0);
        expect(mediaUploadStore.media).toEqual({ id: 1, locale: 'en', mimeType: 'image/jpeg', title: 'test', thumbnails: {}, url: '', adminUrl: '' });
...
test('Calling the "create" method should make a "POST" request to the media update api', () => {
    services_1.resourceRouteRegistry.getUrl.mockReturnValue('/media?locale=en&collection=1');
    const openSpy = jest.fn();
    window.XMLHttpRequest = jest.fn(function () {
        this.open = openSpy;
        this.onload = jest.fn();
    ...
    expect(services_1.resourceRouteRegistry.getUrl).toBeCalledWith('detail', 'media', { collection: 1, locale: 'en' });
...
test('Calling "delete" method should call the "delete" method of the ResourceRequester', () => {
    ...
    services_1.ResourceRequester.delete.mockReturnValue(Promise.resolve());
    const deletePromise = mediaUploadStore.delete();
    expect(services_1.ResourceRequester.delete).toBeCalledWith('media', { id: 2 });
...
test('Calling "deletePreviewImage" method should call the "delete" method of the ResourceRequester', () => {
    ...
    services_1.ResourceRequester.delete.mockReturnValue(Promise.resolve(media));
    const deletePromise = mediaUploadStore.deletePreviewImage();
    expect(services_1.ResourceRequester.delete).toBeCalledWith('media_preview', { id: 2 });
    return deletePromise.then(() => {
...
test('Calling the "updatePreviewImage" method should make a "POST" request to the preview media update api', () => {
    services_1.resourceRouteRegistry.getUrl.mockReturnValue('/media/1/preview?locale=en');
    const openSpy = jest.fn();
    window.XMLHttpRequest = jest.fn(function () {
        this.open = openSpy;
        this.onload = jest.fn();
    ...
    expect(services_1.resourceRouteRegistry.getUrl).toBeCalledWith('detail', 'media_preview', { id: 2, locale: 'en' });
...
test('After the "update" call request was successful the progress will be reset', () => {
    ...
    return updatePromise.then(() => {
        expect(mediaUploadStore.uploading).toEqual(false);
        expect(mediaUploadStore.progress).toEqual(0);
        expect(mediaUploadStore.media).toEqual({ id: 1, locale: 'en', mimeType: 'image/jpeg', title: 'test1', thumbnails: {}, url: '', adminUrl: '' });
        expect(mediaUploadStore.error).toEqual(undefined);
    });
});
