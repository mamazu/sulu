...
export default class MediaUploadStore {
    ...
    constructor(media: Media | null | undefined, locale: IObservableValue<string>) {
        this.media = media;
        this.locale = locale;
    }

    @computed
    get id(): number | null | undefined | string {
        const {media} = this;
    ...
    getThumbnail(size: string): string | null | undefined {
        const {media} = this;
    ...
    create(collectionId: string | number, file: File): Promise<any> {
        const url = resourceRouteRegistry.getUrl(
            'detail',
            RESOURCE_KEY,
            {
    ...
    updatePreviewImage(file: File): Promise<any> {
        const id = this.media ? this.media.id : undefined;
    ...
    deletePreviewImage(): Promise<any> {
        if (!this.id) {
    ...
    @action handleResponse = (media: any) => {
    ...
    upload(file: File, url: string, formName: string): Promise<any> {
