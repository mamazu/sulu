...
test('Navigate to route using state', () => {
    routeRegistry_1.default.getAll.mockReturnValue({
        test: new Route_1.default({
            name: 'test',
            path: '/test',
            type: 'form',
        }),
        page: new Route_1.default({
            name: 'page',
            options: {
                type: 'page',
            },
    ...
    const router = new Router_1.default(history);
    router.navigate('test');
    router.navigate('page', { uuid: 'some-uuid' });
    expect((0, mobx_1.isObservable)(router.route)).toBe(true);
...
test('Redirect to route using state', () => {
    routeRegistry_1.default.getAll.mockReturnValue({
        test1: new Route_1.default({
            name: 'test1',
            type: 'test1',
            path: '/test1',
        }),
        test2: new Route_1.default({
            name: 'test2',
            type: 'test2',
            path: '/test2',
        }),
...
test('Navigate to route with object search parameters using state', () => {
    ...
    const history = (0, history_1.createMemoryHistory)();
    const router = new Router_1.default(history);
    router.navigate('page', { uuid: 'some-uuid', page: 1, filter: { firstName: { eq: 'Max' }, lastName: { eq: 'Mustermann' } } });
    expect((0, mobx_1.isObservable)(router.route)).toBe(true);
    expect(router.route.type).toBe('form');
    expect(router.route.options.type).toBe('page');
    expect(router.attributes.uuid).toBe('some-uuid');
...
test('Navigate to route with array search parameters using state', () => {
    ...
    const history = (0, history_1.createMemoryHistory)();
    const router = new Router_1.default(history);
    router.navigate('page', { uuid: 'some-uuid', page: 1, ids: [1, 2, 3] });
    expect((0, mobx_1.isObservable)(router.route)).toBe(true);
    expect(router.route.type).toBe('form');
    expect(router.route.options.type).toBe('page');
    expect(router.attributes.uuid).toBe('some-uuid');
...
test('Navigate to route with dates in array search parameters using state', () => {
    ...
    const history = (0, history_1.createMemoryHistory)();
    const router = new Router_1.default(history);
    router.navigate('page', { uuid: 'some-uuid', page: 1, dates: [new Date('2020-03-10 00:00'), new Date('2020-03-20 12:00')] });
    expect((0, mobx_1.isObservable)(router.route)).toBe(true);
    expect(router.route.type).toBe('form');
    expect(router.route.options.type).toBe('page');
    expect(router.attributes.uuid).toBe('some-uuid');
...
test('Navigate to route with array nested in object search parameters using state', () => {
    ...
    const history = (0, history_1.createMemoryHistory)();
    const router = new Router_1.default(history);
    router.navigate('page', { uuid: 'some-uuid', page: 1, filter: { ids: [1, 2, 3] } });
    expect((0, mobx_1.isObservable)(router.route)).toBe(true);
    expect(router.route.type).toBe('form');
    expect(router.route.options.type).toBe('page');
    expect(router.attributes.uuid).toBe('some-uuid');
...
test('Navigate to route with date search parameters using state', () => {
    ...
    const history = (0, history_1.createMemoryHistory)();
    const router = new Router_1.default(history);
    router.navigate('page', { uuid: 'some-uuid', page: 1, from: new Date('2020-02-28 00:00') });
    expect((0, mobx_1.isObservable)(router.route)).toBe(true);
    expect(router.route.type).toBe('form');
    expect(router.route.options.type).toBe('page');
    expect(router.attributes.uuid).toBe('some-uuid');
...
test('Navigate to route with string representing invalid date search parameters using state', () => {
    ...
    const history = (0, history_1.createMemoryHistory)();
    const router = new Router_1.default(history);
    router.navigate('page', { uuid: 'some-uuid', page: 1, from: '2020-02-32' });
    expect((0, mobx_1.isObservable)(router.route)).toBe(true);
    expect(router.route.type).toBe('form');
    expect(router.route.options.type).toBe('page');
    expect(router.attributes.uuid).toBe('some-uuid');
...
test('Apply updateAttributesHooks before applying default attributes but after passed attributes', () => {
    routeRegistry_1.default.getAll.mockReturnValue({
        webspace_overview: new Route_1.default({
...
test('Update date observable attribute on route change', () => {
    ...
    const date = mobx_1.observable.box();
    const history = (0, history_1.createMemoryHistory)();
    const router = new Router_1.default(history);
...
test('Update boolean observable attribute on route change', () => {
    ...
    const exclude = mobx_1.observable.box();
    const history = (0, history_1.createMemoryHistory)();
    const router = new Router_1.default(history);
...
test('Update attribute containing an observable array on route change', () => {
    ...
    const filter = mobx_1.observable.box({
        accountId: (0, mobx_1.observable)([1, 2]),
    });
    const history = (0, history_1.createMemoryHistory)();
    const router = new Router_1.default(history);
...
test('Binding should update passed observable', () => {
    ...
    const value = mobx_1.observable.box();
    const history = (0, history_1.createMemoryHistory)();
    const router = new Router_1.default(history);
...
test('Binding should update URL with fixed attributes', () => {
    ...
    const uuid = mobx_1.observable.box(1);
    const history = (0, history_1.createMemoryHistory)();
    const router = new Router_1.default(history);
...
test('Binding should update URL with fixed attributes as string if not a number', () => {
    ...
    const uuid = mobx_1.observable.box('old-uuid');
    const history = (0, history_1.createMemoryHistory)();
    const router = new Router_1.default(history);
...
test('Set state to undefined if parameter is removed from URL', () => {
    ...
    const value = mobx_1.observable.box(5);
    const history = (0, history_1.createMemoryHistory)();
    const router = new Router_1.default(history);
    router.bind('page', value);
    history.push('/list');
...
test('Navigate to child route using state', () => {
    const formRoute = new Route_1.default({
    ...
    const detailsRoute = new Route_1.default({
    ...
    const taxonomyRoute = new Route_1.default({
...
test('Cancel navigation if an updateRouteHook returns false', () => {
    const webspaceOverviewRoute = new Route_1.default({
        name: 'webspace_overview',
        type: 'webspace_overview',
        path: '/webspace/:webspace/:locale',
        attributeDefaults: {
    ...
    expect(updateRouteHook1).toBeCalledWith(webspaceOverviewRoute, { locale: 'en', sortOrder: 'desc', webspace: 'webspace1' }, router.navigate);
...
test('Consider priority when cancelling a navigation', () => {
    ...
    router.navigate('webspace_overview', { locale: 'en' });
    expect(updateRouteHook2).toBeCalledWith(webspaceOverviewRoute, { locale: 'en', sortOrder: 'desc', webspace: 'webspace1' }, router.navigate);
...
test('Redirect if all updateRouteHooks return true', () => {
    ...
    expect(updateRouteHook1).toBeCalledWith(webspaceOverviewRoute, { locale: 'en', sortOrder: 'desc', webspace: 'webspace1' }, router.redirect);
    expect(updateRouteHook2).toBeCalledWith(webspaceOverviewRoute, { locale: 'en', sortOrder: 'desc', webspace: 'webspace1' }, router.redirect);
    expect(router.route).toEqual(webspaceOverviewRoute);
    expect(router.attributes).toEqual({ locale: 'en', sortOrder: 'desc', webspace: 'webspace1' });
    expect(history.location.pathname).toBe('/webspace/webspace1/en');
});
test('Restore if all updateRouteHooks return true', () => {
    ...
    expect(updateRouteHook1).toBeCalledWith(webspaceOverviewRoute, { locale: 'en', sortOrder: 'desc', webspace: 'webspace1' }, router.restore);
    expect(updateRouteHook2).toBeCalledWith(webspaceOverviewRoute, { locale: 'en', sortOrder: 'desc', webspace: 'webspace1' }, router.restore);
    expect(router.route).toEqual(webspaceOverviewRoute);
    expect(router.attributes).toEqual({ locale: 'en', sortOrder: 'desc', webspace: 'webspace1' });
    expect(history.location.pathname).toBe('/webspace/webspace1/en');
});
