...
function createTypeToolbarAction(options = {}) {
    ...
    return new TypeToolbarAction_1.default(resourceFormStore, form, router, [], options, resourceStore);
}
...
test('Change the type of the FormStore when FormStore is not dirty and another type is selected', () => {
    ...
    if (toolbarItemConfig.type !== 'select') {
        throw new Error('The returned toolbar item must be of type "select", but "' + toolbarItemConfig.type + '" was given!');
    }
...
test('Display warning dialog when FormStore is dirty and another type is selected', () => {
    ...
    expect((0, enzyme_1.mount)(typeToolbarAction.getNode()).instance().props).toEqual(expect.objectContaining({
...
test('Change the type of the FormStore when warning dialog is confirmed', () => {
    ...
    toolbarItemConfig.onChange('homepage');
    expect(typeToolbarAction.resourceFormStore.changeType).not.toBeCalled();
    expect((0, enzyme_1.mount)(typeToolbarAction.getNode()).instance().props.open).toBeTruthy();
    (0, enzyme_1.mount)(typeToolbarAction.getNode()).instance().props.onConfirm();
    expect(typeToolbarAction.resourceFormStore.changeType).toBeCalledWith('homepage');
    expect((0, enzyme_1.mount)(typeToolbarAction.getNode()).instance().props.open).toBeFalsy();
});
test('Do not change the type of the FormStore when warning dialog is canceled', () => {
    ...
    expect(typeToolbarAction.resourceFormStore.changeType).not.toBeCalled();
    expect((0, enzyme_1.mount)(typeToolbarAction.getNode()).instance().props.open).toBeTruthy();
    (0, enzyme_1.mount)(typeToolbarAction.getNode()).instance().props.onCancel();
    expect(typeToolbarAction.resourceFormStore.changeType).not.toBeCalled();
    expect((0, enzyme_1.mount)(typeToolbarAction.getNode()).instance().props.open).toBeFalsy();
});
