...
const Form_1 = __importDefault(require("../../../../views/Form"));
...
function createTogglerToolbarAction(resourceKey, options) {
    const resourceStore = new ResourceStore_1.default(resourceKey);
    const resourceFormStore = new Form_2.ResourceFormStore(resourceStore, resourceKey);
    ...
    return new TogglerToolbarAction_1.default(resourceFormStore, form, router, [], options, resourceStore);
}
...
test.each([
    ...
    ['lock', 'locked', 'users'],
])('Call ResourceRequester with correct parameters when resource is not activated and toggler is clicked', (action, property, resourceKey) => {
    const promise = Promise.resolve({ [property]: true });
    services_1.ResourceRequester.post.mockReturnValue(promise);
    ...
    expect(services_1.ResourceRequester.post).toBeCalledWith(resourceKey, undefined, { action, id: 1234, locale: 'de' });
    return promise.then(() => {
...
test.each([
    ...
    ['unlock', 'locked', 'users'],
])('Call ResourceRequester with correct parameters when resource is already activated and toggler is clicked', (action, property, resourceKey) => {
...
test('Return item config with loading toggler during request', () => {
    ...
    services_1.ResourceRequester.post.mockReturnValue(publishPromise);
    const toolbarAction = createTogglerToolbarAction('test', { label: 'Test', property: 'published', activate: 'publish' });
    toolbarAction.resourceFormStore.resourceStore.loading = false;
    toolbarAction.resourceFormStore.resourceStore.data.id = 1234;
    toolbarAction.resourceFormStore.resourceStore.data.published = false;
    expect(toolbarAction.getToolbarItemConfig()).toEqual(expect.objectContaining({
