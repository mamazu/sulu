...
jest.mock('../../../../stores/ResourceStore', () => jest.fn(function(resourceKey: any) {
...
function createTogglerToolbarAction(resourceKey: any, options: {
    [key: string]: unknown
}) {
...
test.each([
    ['sulu_admin.publish'],
    ['sulu_admin.lock'],
])('Return item config with label "%s"', (label: any) => {
...
test.each([
    ['published'],
    ['locked'],
])('Return correct value when resource is already in activated state for property "%s"', (property: any) => {
...
test.each([
    ...
    (action: any, property: any, resourceKey: any) => {
...
test('Push error to form view on failed request', (done: any) => {
