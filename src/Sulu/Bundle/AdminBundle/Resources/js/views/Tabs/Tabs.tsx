...
import Badge, {BadgeOptions} from '../../containers/Badge';
...
type Props<T> = (ViewProps) & {
    children?: (arg1: T) => Element<any> | null,
    childrenProps: T,
    header?: Node,
...
class Tabs<T> extends React.Component<Props<T>> {
    ...
    @computed
    get tabRouteWithHighestPriority(): any {
    ...
    @computed
    get sortedTabRoutes(): Array<Route> {
    ...
    handleSelect = (index: number) => {
        ...
        const filteredAttributes = routerAttributesToBlacklist
            ? Object.keys(router.attributes)
                ...
                .reduce<Record<string, any>>((attributes, key) => {
                    ...
                    {showTabs &&
                        ...
                        >
                            {this.sortedTabRoutes.map((tabRoute) => {
                                ...
                                const badges = (Object.values(tabBadges) as any).map(
