...
const BlockCollection_1 = __importDefault(require("../BlockCollection"));
const SortableBlockList_1 = __importDefault(require("../SortableBlockList"));
const clipboard_1 = __importDefault(require("../../../utils/clipboard/clipboard"));
beforeEach(() => {
    BlockCollection_1.default.idCounter = 0;
});
jest.mock('../../../utils/Translator', () => ({
    translate: jest.fn((key) => key),
}));
test('Should render a fully filled block list', () => {
    expect((0, enzyme_1.render)(<BlockCollection_1.default defaultType="editor" icons={[[], ['su-eye']]} maxOccurs={3} onChange={jest.fn()} renderBlockContent={jest.fn()} value={[{ content: 'Test 1', type: 'editor' }, { content: 'Test 2', type: 'editor' }]}/>)).toMatchSnapshot();
});
test('Should render a non-movable block list', () => {
    expect((0, enzyme_1.render)(<BlockCollection_1.default collapsable={false} defaultType="editor" movable={false} onChange={jest.fn()} renderBlockContent={jest.fn()} value={[{ content: 'Test 1', type: 'editor' }, { content: 'Test 2', type: 'editor' }]}/>)).toMatchSnapshot();
});
test('Should render a disabled block list', () => {
    const blockCollection = (0, enzyme_1.mount)(<BlockCollection_1.default defaultType="editor" disabled={true} onChange={jest.fn()} renderBlockContent={jest.fn()} value={[{ content: 'Test 1', type: 'editor' }, { content: 'Test 2', type: 'editor' }]}/>);
...
test('Should mark the add button disabled if maxOccurs is reached', () => {
    const blockCollection = (0, enzyme_1.shallow)(<BlockCollection_1.default defaultType="editor" maxOccurs={2} onChange={jest.fn()} renderBlockContent={jest.fn()} value={[{ content: 'Test 1', type: 'editor' }, { content: 'Test 2', type: 'editor' }]}/>);
...
test('Should render add button with the given addButtonText', () => {
    const blockCollection = (0, enzyme_1.shallow)(<BlockCollection_1.default addButtonText="custom-add-button-text" defaultType="editor" maxOccurs={2} onChange={jest.fn()} renderBlockContent={jest.fn()} value={[{ content: 'Test 1', type: 'editor' }, { content: 'Test 2', type: 'editor' }]}/>);
...
test('Should render paste button if clipboard contains a block', () => {
    ...
    clipboard_1.default.set('blocks', [{ content: 'Test 3', type: 'editor' }]);
...
test('Should render paste button with the given pasteButtonText', () => {
    clipboard_1.default.set('blocks', [{ content: 'Test 3', type: 'editor' }]);
    const blockCollection = (0, enzyme_1.shallow)(<BlockCollection_1.default defaultType="editor" maxOccurs={2} onChange={jest.fn()} pasteButtonText="custom-paste-button-text" renderBlockContent={jest.fn()} value={[{ content: 'Test 1', type: 'editor' }, { content: 'Test 2', type: 'editor' }]}/>);
    expect(blockCollection.find('Button[icon="su-copy"]').exists()).toBeTruthy();
...
test('Should add at least the minOccurs amount of blocks', () => {
    ...
    (0, enzyme_1.shallow)(<BlockCollection_1.default defaultType="editor" minOccurs={2} onChange={changeSpy} renderBlockContent={jest.fn()} value={value}/>);
...
test('Should add at least the minOccurs amount of blocks with types', () => {
    ...
    (0, enzyme_1.shallow)(<BlockCollection_1.default defaultType="editor" minOccurs={2} onChange={changeSpy} renderBlockContent={jest.fn()} types={{ default: 'Default', editor: 'Editor' }} value={value}/>);
    expect(changeSpy).toBeCalledWith([
...
test('Choosing a different type should call the onChange callback', () => {
    ...
    const blockCollection = (0, enzyme_1.mount)(<BlockCollection_1.default defaultType="editor" onChange={changeSpy} renderBlockContent={renderBlockContent} types={{ type1: 'Type 1', type2: 'Type2' }} value={value}/>);
...
test('Should allow to expand blocks', () => {
    const blockCollection = (0, enzyme_1.mount)(<BlockCollection_1.default defaultType="editor" onChange={jest.fn()} renderBlockContent={jest.fn()} value={[{ content: 'Test 1', type: 'editor' }, { content: 'Test 2', type: 'editor' }]}/>);
...
test('Should allow to reorder blocks by using drag and drop', () => {
    ...
    const blockCollection = (0, enzyme_1.mount)(<BlockCollection_1.default defaultType="editor" onChange={changeSpy} onSortEnd={sortEndSpy} renderBlockContent={jest.fn()} value={value}/>);
    blockCollection.find('Block').at(0).simulate('click');
    ...
    blockCollection.find(SortableBlockList_1.default).prop('onSortEnd')({ newIndex: 2, oldIndex: 0 });
    expect(changeSpy).toBeCalledWith([
...
test('Should add a new block between existing blocks', () => {
    ...
    const blockCollection = (0, enzyme_1.mount)(<BlockCollection_1.default defaultType="editor" onChange={changeSpy} renderBlockContent={jest.fn()} value={value}/>);
...
test('Should throw an exception if a new block is added and the maximum has already been reached', () => {
    const changeSpy = jest.fn();
    const value = [{ content: 'Test 1', type: 'editor' }, { content: 'Test 2', type: 'editor' }];
    const blockCollection = (0, enzyme_1.shallow)(<BlockCollection_1.default defaultType="editor" maxOccurs={2} onChange={changeSpy} renderBlockContent={jest.fn()} value={value}/>);
...
test('Should paste block between existing blocks', () => {
    clipboard_1.default.set('blocks', [{ content: 'Clipboard', type: 'editor' }]);
    const changeSpy = jest.fn();
    const value = [{ content: 'Test 1', type: 'editor' }, { content: 'Test 2', type: 'editor' }];
    const blockCollection = (0, enzyme_1.mount)(<BlockCollection_1.default defaultType="editor" onChange={changeSpy} renderBlockContent={jest.fn()} value={value}/>);
...
test('Should paste block with default type if type of block in clipboard block is not known', () => {
    clipboard_1.default.set('blocks', [{ content: 'Clipboard', type: 'unkown-type' }]);
    const changeSpy = jest.fn();
    const value = [{ content: 'Test 1', type: 'editor' }, { content: 'Test 2', type: 'editor' }];
    const blockCollection = (0, enzyme_1.mount)(<BlockCollection_1.default defaultType="editor" onChange={changeSpy} renderBlockContent={jest.fn()} value={value}/>);
    blockCollection.find('Button[icon="su-copy"]').last().simulate('click');
...
test('Should pass duplicate action that allows to duplicate an existing block', () => {
    ...
    const value = (0, mobx_1.observable)([{ content: 'Test 1', type: 'editor' }, { content: 'Test 2', type: 'editor' }]);
    ...
    expect(blockActions).toContainEqual(expect.objectContaining({
        type: 'button',
        icon: 'su-duplicate',
...
test('Should not pass duplicate action to Block component if maxOccurs limit is reached', () => {
    ...
    const blockCollection = (0, enzyme_1.mount)(<BlockCollection_1.default defaultType="editor" maxOccurs={2} onChange={jest.fn()} renderBlockContent={jest.fn()} value={value}/>);
    const blockActions = blockCollection.find('Block').at(0).prop('actions');
...
test('Should not pass remove action to Block component if minOccurs limit is reached', () => {
    const value = [{ content: 'Value 1', type: 'editor' }, { content: 'Value 2', type: 'editor' }];
    const blockCollection = (0, enzyme_1.mount)(<BlockCollection_1.default defaultType="editor" minOccurs={2} onChange={jest.fn()} renderBlockContent={jest.fn()} value={value}/>);
    const blockActions = blockCollection.find('Block').at(0).prop('actions');
    expect(blockActions).not.toContainEqual(expect.objectContaining({
        label: 'sulu_admin.delete',
    }));
...
test('Should throw an exception if a block is removed and minOccurs limit is reached', () => {
    const changeSpy = jest.fn();
    const value = [{ content: 'Test 1', type: 'editor' }, { content: 'Test 2', type: 'editor' }];
    const blockCollection = (0, enzyme_1.shallow)(<BlockCollection_1.default defaultType="editor" minOccurs={2} onChange={changeSpy} renderBlockContent={jest.fn()} value={value}/>);
...
test('Should pass copy action that allows to cut an existing block into the clipboard', () => {
    ...
    clipboard_1.default.observe('blocks', clipboardSpy);
    const value = (0, mobx_1.observable)([{ content: 'Test 1', type: 'editor' }, { content: 'Test 2', type: 'editor' }]);
    const blockCollection = (0, enzyme_1.mount)(<BlockCollection_1.default defaultType="editor" onChange={jest.fn()} renderBlockContent={jest.fn()} value={value}/>);
    const blockActions = blockCollection.find('Block').at(0).prop('actions');
    expect(blockActions).toContainEqual(expect.objectContaining({
        type: 'button',
        icon: 'su-copy',
...
test('Should call onSettingsClick callback when settings icon is clicked', () => {
    ...
    const value = (0, mobx_1.observable)([{ content: 'Test 1', type: 'editor' }, { content: 'Test 2', type: 'editor' }]);
    const blockCollection = (0, enzyme_1.mount)(<BlockCollection_1.default defaultType="editor" onChange={jest.fn()} onSettingsClick={settingsClickSpy} renderBlockContent={jest.fn()} value={value}/>);
    blockCollection.find('Block').at(0).simulate('click');
    blockCollection.find('Block').at(1).simulate('click');
...
test('Should apply renderBlockContent before rendering the block content', () => {
    ...
    const blockCollection = (0, enzyme_1.mount)(<BlockCollection_1.default defaultType="editor" onChange={jest.fn()} renderBlockContent={renderBlockContent} value={value}/>);
    blockCollection.find('Block').at(0).simulate('click');
    blockCollection.find('Block').at(1).simulate('click');
...
test('Should apply renderBlockContent before rendering the block content including the type', () => {
    ...
    const renderBlockContent = jest.fn().mockImplementation((value, type) => prefix + value.content + (type ? typePrefix + type : ''));
    const types = {
        type1: 'Type 1',
        type2: 'Type 2',
    };
    const blockCollection = (0, enzyme_1.mount)(<BlockCollection_1.default defaultType="editor" onChange={jest.fn()} renderBlockContent={renderBlockContent} types={types} value={value}/>);
    blockCollection.find('Block').at(0).simulate('click');
    blockCollection.find('Block').at(1).simulate('click');
...
test('Should adjust expandedBlocks and generatedBlockIds after updating the value variable with fewer entries', () => {
    ...
    const blockCollection = (0, enzyme_1.mount)(<BlockCollection_1.default defaultType="editor" onChange={jest.fn()} renderBlockContent={jest.fn()} types={types} value={value}/>);
...
test('Copy selected blocks via the BlockToolbar', () => {
    ...
    const blockCollection = (0, enzyme_1.mount)(<BlockCollection_1.default defaultType="editor" onChange={changeSpy} renderBlockContent={jest.fn()} types={types} value={value}/>);
    const selectMultipleButton = blockCollection.find('button.selectMultipleButton');
    selectMultipleButton.simulate('click');
    const blockToolbar = blockCollection.find('BlockToolbar');
    const selectionHandles = blockCollection.find('SelectionHandle');
