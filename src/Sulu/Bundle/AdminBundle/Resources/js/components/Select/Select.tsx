...
type Props<T> = (SelectProps<T>) & {
    ...
    displayValue: string,
    isOptionSelected: (option: Element<Flow.Class<Option<T>>>) => boolean,
    onClose?: () => void,
...
class Select<T> extends React.Component<Props<T>> {
    ...
    @observable
    displayValueRef: ElementRef<'button'> | null | undefined;

    @observable
    selectedOptionRef: ElementRef<'li'> | null | undefined;

    @observable
    buttonRefsByIndex: Map<number, ElementRef<'button'>> = new Map();

    @observable
    searchText: string = '';

    @observable
    focusedElementIndex: number = -1;

    @observable
    open: boolean = false;

    @computed
    get buttonTextsByIndex(): Map<number, string> {
        return Array.from(this.buttonRefsByIndex.entries())
            .reduce((buttonTextsByIndex, [index, ref]: [any, any]) => {
    ...
    @computed
    get availableButtonIndices(): number[] {
    ...
    @action setDisplayValueRef = (ref?: ElementRef<'button'> | null) => {
        if (ref) {
            this.displayValueRef = ref;
        }
    };

    @action setSelectedOptionRef = (ref: ElementRef<'li'> | null | undefined, selected: boolean) => {
        if (!this.selectedOptionRef || (ref && selected)) {
            this.selectedOptionRef = ref;
        }
    };

    setButtonRef = (index: number) => action((ref?: ElementRef<'button'> | null) => {
        if (ref) {
    ...
    @action appendSearchText = (searchText: string) => {
        ...
        const hit = entries.find(([, text]: [any, any]) => text.toLowerCase().startsWith(this.searchText.toLowerCase()));
    ...
    cloneOption(originalOption: Element<Flow.Class<Option<T>>>, index: number): Element<Flow.Class<Option<T>>> {
