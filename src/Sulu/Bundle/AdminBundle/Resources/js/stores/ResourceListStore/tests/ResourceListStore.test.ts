...
test('Patch and delete items using the ResourceRequester', () => {
    ...
    let deletePromiseResolve: any;
    const deleteListPromise = new Promise((resolve: any) => deletePromiseResolve = resolve);
    ResourceRequester.deleteList.mockReturnValue(deleteListPromise);

    let patchPromiseResolve: any;
    const patchListPromise = new Promise((resolve: any) => patchPromiseResolve = resolve);
