...
test('Should be marked as initialized after loading the data', () => {
    ...
    const resourceStore = new ResourceStore_1.default('snippets', '1');
...
test('Should be marked as initialized immediately if a new resource is created', () => {
    const resourceStore = new ResourceStore_1.default('snippets');
    expect(resourceStore.initialized).toBe(true);
});
...
test('Should load the data with the ResourceRequester', () => {
    ...
    ResourceRequester_1.default.get.mockReturnValue(promise);
    const resourceStore = new ResourceStore_1.default('snippets', '3', { locale: mobx_1.observable.box('en') }, { test: 10 });
    expect(ResourceRequester_1.default.get).toBeCalledWith('snippets', { id: '3', locale: 'en', test: 10 });
    return promise.then(() => {
...
test('Should load without locale the data with the ResourceRequester', () => {
    const promise = Promise.resolve({ value: 'Value' });
    ResourceRequester_1.default.get.mockReturnValue(promise);
    const resourceStore = new ResourceStore_1.default('snippets', '3');
    const oldData = resourceStore.data;
    expect(ResourceRequester_1.default.get).toBeCalledWith('snippets', { id: '3' });
    return promise.then(() => {
        expect(resourceStore.data).toEqual({ value: 'Value' });
...
test('Should load with the idQueryParameter and reset after successful load', () => {
    ...
    ResourceRequester_1.default.get.mockReturnValue(promise);
    const resourceStore = new ResourceStore_1.default('users', 2, {}, {}, 'contactId');
    const oldData = resourceStore.data;
    expect(resourceStore.idQueryParameter).toEqual('contactId');
    expect(ResourceRequester_1.default.get).toBeCalledWith('users', { contactId: 2 });
    return promise.then(() => {
...
test('Should not load the data with the ResourceRequester if no resource-id is provided', () => {
    const promise = Promise.resolve({ value: 'Value' });
    ResourceRequester_1.default.get.mockReturnValue(promise);
    new ResourceStore_1.default('snippets', null, { locale: mobx_1.observable.box() });
...
test('Should not load the data with the ResourceRequester if locale should be provided but is not', () => {
    const promise = Promise.resolve({ value: 'Value' });
    ResourceRequester_1.default.get.mockReturnValue(promise);
    new ResourceStore_1.default('snippets', '3', { locale: mobx_1.observable.box() });
    expect(ResourceRequester_1.default.get).not.toBeCalled();
});
test('Should load the data with the ResourceRequester if a reload is requested', () => {
    const promise1 = Promise.resolve({ value: 'Value' });
    ResourceRequester_1.default.get.mockReturnValue(promise1);
    const resourceStore = new ResourceStore_1.default('snippets', '3', { locale: mobx_1.observable.box('en') }, { test: 10 });
    expect(ResourceRequester_1.default.get).toBeCalledWith('snippets', { id: '3', locale: 'en', test: 10 });
    return promise1.then(() => {
        expect(resourceStore.data).toEqual({ value: 'Value' });
        const promise2 = Promise.resolve({ value: 'new Value' });
        ResourceRequester_1.default.get.mockReturnValue(promise2);
...
test('Loading flag should be set to true when loading', () => {
    ResourceRequester_1.default.get.mockReturnValue(Promise.resolve({}));
    const resourceStore = new ResourceStore_1.default('snippets', '1', { locale: mobx_1.observable.box('en') });
...
test('Saving flag should be set to true when saving', () => {
    ResourceRequester_1.default.put.mockReturnValue(Promise.resolve({}));
    const resourceStore = new ResourceStore_1.default('snippets', '1', { locale: mobx_1.observable.box('en') });
...
test('Saving flag should be set to false when saving has finished', () => {
    const promise = Promise.resolve({});
    ResourceRequester_1.default.put.mockReturnValue(promise);
    const resourceStore = new ResourceStore_1.default('snippets', '1', { locale: mobx_1.observable.box('en') });
...
test('Save the store should send a PUT request', () => {
    ResourceRequester_1.default.put.mockReturnValue(Promise.resolve({}));
    const resourceStore = new ResourceStore_1.default('snippets', '3', { locale: mobx_1.observable.box() });
    ...
    expect(ResourceRequester_1.default.put).toBeCalledWith('snippets', { title: 'Title' }, { id: '3', locale: 'de', test: 10 });
});
test('Save the store should send a PUT request without a locale', () => {
    ResourceRequester_1.default.post.mockReturnValue(Promise.resolve({}));
    const resourceStore = new ResourceStore_1.default('snippets', null, { locale: mobx_1.observable.box() });
    if (!resourceStore.locale) {
        throw new Error('The resourceStore should have a locale');
    }
    resourceStore.locale.set('de');
    ...
    resourceStore.dirty = false;
    resourceStore.save({ test: 10 });
    expect(ResourceRequester_1.default.post).toBeCalledWith('snippets', { title: 'Title' }, { locale: 'de', test: 10 });
});
...
test('Save the store without an id should send a POST request without a locale', () => {
    ResourceRequester_1.default.post.mockReturnValue(Promise.resolve({}));
    const resourceStore = new ResourceStore_1.default('snippets', null, {});
    resourceStore.data = { title: 'Title' };
    resourceStore.dirty = false;
    resourceStore.save({ test: 10 });
    expect(ResourceRequester_1.default.post).toBeCalledWith('snippets', { title: 'Title' }, { test: 10 });
});
test('Saving and dirty flag should be set to false when creating has failed', (done) => {
    ...
    const resourceStore = new ResourceStore_1.default('snippets', undefined, { locale: mobx_1.observable.box() });
    if (!resourceStore.locale) {
        throw new Error('The resourceStore should have a locale');
    }
    ...
    return savePromise.catch((promiseError) => {
        ...
        (0, mobx_1.when)(() => !resourceStore.saving, () => {
            expect(resourceStore.saving).toBe(false);
            expect(resourceStore.dirty).toBe(true);
...
test('Deleting without delete_locale should not throw error when locale is undefined', () => {
    ResourceRequester_1.default.delete.mockReturnValue(Promise.resolve({}));
    const resourceStore = new ResourceStore_1.default('snippets', '1', { locale: mobx_1.observable.box() });
    resourceStore.data = { id: 1 };
    expect(() => {
...
test('Deleting flag and id should be reset to false when deleting has finished', () => {
    const promise = Promise.resolve({});
    ResourceRequester_1.default.delete.mockReturnValue(promise);
    const resourceStore = new ResourceStore_1.default('snippets', '1', { locale: mobx_1.observable.box('en') });
    resourceStore.data = { id: 1 };
    const oldData = resourceStore.data;
    resourceStore.deleting = false;
...
test('Calling the delete method should send a DELETE request', () => {
    ...
    resourceStore.dirty = false;
    resourceStore.delete();
    expect(ResourceRequester_1.default.delete).toBeCalledWith('snippets', { id: 3 });
});
test('Calling the delete method with options should send a DELETE request', () => {
    ...
    expect(ResourceRequester_1.default.delete).toBeCalledWith('snippets', { id: 3, test: 'value' });
});
...
test('Moving flag and id should be reset to false when moving has finished', () => {
    ...
    return promise.then(() => {
        ...
        expect(ResourceRequester_1.default.get).toBeCalledWith('snippets', { id: 1, locale: 'en' });
        expect(resourceStore.data).toBe(oldData);
    });
});
test('Calling the move method should send a POST request', () => {
    ...
    expect(ResourceRequester_1.default.post).toBeCalledWith('snippets', undefined, { action: 'move', destination: 9, id: 3, locale: undefined });
});
test('Calling the move method should send a POST request with locale', () => {
    ResourceRequester_1.default.post.mockReturnValue(Promise.resolve({}));
    const resourceStore = new ResourceStore_1.default('snippets', 3, { locale: mobx_1.observable.box('de') });
    resourceStore.data = { id: 3, title: 'Title' };
    resourceStore.move(9);
    expect(ResourceRequester_1.default.post).toBeCalledWith('snippets', undefined, { action: 'move', destination: 9, id: 3, locale: 'de' });
});
test('Saving flag should be set to true when saving', () => {
    ResourceRequester_1.default.put.mockReturnValue(Promise.resolve({}));
    const resourceStore = new ResourceStore_1.default('snippets', '1', { locale: mobx_1.observable.box() });
...
test('Saving and dirty flag should be set to false when updating has failed', (done) => {
    ...
    ResourceRequester_1.default.get.mockReturnValue(loadingPromise);
    ResourceRequester_1.default.put.mockReturnValue(promise);
    const resourceStore = new ResourceStore_1.default('snippets', '1', { locale: mobx_1.observable.box() });
    if (!resourceStore.locale) {
        throw new Error('The resourceStore should have a locale');
...
test('Copy the content from different locale', () => {
    const resourceStore = new ResourceStore_1.default('pages', 4, { locale: mobx_1.observable.box('en') }, { webspace: 'sulu_io' });
...
test('Copying the content from different locale should fail if no id is given', () => {
    const resourceStore = new ResourceStore_1.default('pages');
...
test('Copying the content from different locale should fail if no locale is given', () => {
    const resourceStore = new ResourceStore_1.default('pages', 4);
...
test('Cloning should return a new instance of the ResourceStore with same data', () => {
    ...
    expect((0, mobx_1.toJS)(clonedResourceStore.data)).toEqual({ title: 'Title' });
    expect(clonedResourceStore.data).not.toBe(resourceStore.data);
    expect((0, mobx_1.toJS)(clonedResourceStore)).not.toBe(resourceStore);
    expect(ResourceRequester_1.default.get).toHaveBeenCalledTimes(1);
});
test('Cloning should return a new instance of the ResourceStore which updates after changing locale', () => {
    ...
    const resourceStore = new ResourceStore_1.default('snippets', '1', { locale });
    resourceStore.data = { title: 'Title' };
    resourceStore.loading = false;
    const clonedResourceStore = resourceStore.clone();
    expect((0, mobx_1.toJS)(clonedResourceStore.data)).toEqual({ title: 'Title' });
...
test('Cloning during loading should return a new instance of the ResourceStore with same data', () => {
    ...
    ResourceRequester_1.default.get.mockReturnValue(snippetPromise);
    const resourceStore = new ResourceStore_1.default('snippets', '1', {});
    const clonedResourceStore = resourceStore.clone();
    ...
    return snippetPromise.then(() => {
        expect((0, mobx_1.toJS)(resourceStore.data)).toEqual({ title: 'Snippet' });
        expect((0, mobx_1.toJS)(clonedResourceStore.data)).toEqual({ title: 'Snippet' });
        expect(clonedResourceStore.data).not.toBe(resourceStore.data);
        expect((0, mobx_1.toJS)(clonedResourceStore)).not.toBe(resourceStore);
        expect(ResourceRequester_1.default.get).toHaveBeenCalledTimes(1);
    });
...
test('Should set the internal id if id is set using set', () => {
    const resourceStore = new ResourceStore_1.default('media');
    expect(resourceStore.id).toBe(undefined);
