...
test('Should be marked as forbidden if loading failed with 403 and reset if next request succeeds', (done: any) => {
...
test('Should be marked as notFound if loading failed with 404 and reset if next request succeeds', (done: any) => {
...
test('Should be marked as unexpectedError if loading failed with 500 and reset if next request succeeds', (done: any) => {
...
test('Saving and dirty flag should be set to false when creating has failed', (done: any) => {
    ...
    return savePromise.catch((promiseError: any) => {
...
test('Response should be returned when updating', () => {
    ...
    return resourceStore.save().then((responseData: any) => {
...
test('Saving and dirty flag should be set and data should be updated to false when saving has finished', () => {
    const data = {changed: 'later'} as const;
    const promise = Promise.resolve(data);
    ResourceRequester.put.mockReturnValue(promise);
    const resourceStore = new ResourceStore('snippets', '1', {locale: observable.box()});
    const oldData = resourceStore.data;
...
test('Saving and dirty flag should be set to false when updating has failed', (done: any) => {
    const error = new Error('An error occured!');
    const promise = Promise.reject(error);
...
test('Copy the content from different locale', () => {
    ...
    const germanContent = {id: 3, content: 'new content'} as const;
...
test('Cloning during loading should return a new instance of the ResourceStore with same data', () => {
    ...
    let snippetResolve: any;
    const snippetPromise = new Promise((resolve: any) => snippetResolve = resolve);
