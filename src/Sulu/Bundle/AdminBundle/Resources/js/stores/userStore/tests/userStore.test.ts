...
test('Should clear the user store', () => {
    const user = {id: 1, locale: 'cool_locale', settings: {}, username: 'test', roles: []} as const;
    const contact = {id: 12, avatar: undefined, firstName: 'Firsti', lastName: 'Lasti', fullName: 'Firsti Lasti'} as const;
...
test('Should two factor login without initializing when it`s the same user', () => {
    const user = {id: 1, locale: 'en', settings: {}, username: 'test', roles: []} as const;
    const loginPromise = Promise.resolve({
        username: 'test',
        completed: true,
    });
