...
test('Render collapsed blocks with block previews', () => {
    ...
    blockPreviewTransformerRegistry.has.mockImplementation((key: any) => {
        switch (key) {
            case 'text_line':
                return true;
            default:
    ...
    blockPreviewTransformerRegistry.get.mockImplementation((key: any) => {
        switch (key) {
            case 'text_line':
                return {
                    transform: function Transformer(value: any) {
                        return <p>{value}</p>;
                    },
                };
        }
...
test('Render block with schema and error on fields already being modified', () => {
    ...
    formInspector.isFieldModified.mockImplementation((dataPath: any) => {
...
test('Throw error if passed add_button_text schema option is not a string', () => {
    ...
    expect(() => shallow(
        <FieldBlocks
            ...
            defaultType="editor"
            formInspector={formInspector}
            schemaOptions={{add_button_text: {name: 'add_button_text', title: ([] as any)}}}
            types={types}
            value={[]}
        />
