...
class FieldBlocks extends React.Component<FieldTypeProps<Array<BlockEntry>>> {
    @observable
    openedBlockSettingsIndex: number | null | undefined;
    @observable
    blockSettingsFormStore: FormStoreInterface | null | undefined;
    @observable
    value: any;
    oldIconValue: any | null | undefined;
    ...
    @computed get iconsMapping() {
        ...
        return Object.keys(settingsSchema).reduce<Record<string, any>>(iconMappingReducerCreator(), {});
    }

    @computed
    get icons(): Array<Array<string>> {
    ...
    handleBlockChange = (index: number, name: string, value: any) => {
    ...
    getBlockSchemaType = (type?: string | null) => {
    ...
    renderBlockContent = (value: any, type: string, index: number, expanded: boolean) => {
        return expanded
            ? this.renderExpandedBlockContent(value, type, index)
            : this.renderCollapsedBlockContent(value, type, index);
    };

    renderExpandedBlockContent = (value: any, type: string, index: number) => {
        const {
            data,
            dataPath,
            error,
        ...
        const errors = (toJS(error) as BlockError | null | undefined);
    ...
    renderCollapsedBlockContent = (value: any, type: string, index: number) => {
        const blockSchemaType = this.getBlockSchemaType(type);
    ...
    removeSections(blockSchemaTypeForm: any) {
        let filteredForm: Record<string, any> = {};
    ...
    render() {
        ...
        const blockTypes = Object.keys(types).reduce<Record<string, any>>((blockTypes, current) => {
