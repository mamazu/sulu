...
type Props<T extends string | number> = {
    ...
    onChange: (values: Array<T>, valueObjects?: Array<any>) => void,
    onClose?: () => void,
...
@observer
class ResourceMultiSelect<T extends string | number> extends React.Component<Props<T>> {
    static defaultProps = {
        disabled: false,
        idProperty: 'id',
        requestParameters: {},
    ...
    @action createResourceListStore = () => {
        ...
        const parameters = {limit: '', ...requestParameters} as const;

        this.resourceListStore = new ResourceListStore(resourceKey, parameters, idProperty);
    };

    handleChange: (arg1: Array<T>) => void = (values) => {
        const {
            onChange,
            idProperty,
        } = this.props;
    ...
    render() {
        ...
        return (
            <MultiSelectComponent
                ...
                values={values}
            >
                {this.resourceListStore.data.map((object, index) => (<MultiSelectComponent.Option key={index} value={object[idProperty]}>
                    {object[displayProperty]}
                </MultiSelectComponent.Option> as Element<typeof MultiSelectComponent.Option>))}
            </MultiSelectComponent>
        );
    }
}
