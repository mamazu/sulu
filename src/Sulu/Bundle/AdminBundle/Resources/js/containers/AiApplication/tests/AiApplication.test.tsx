...
describe('AiApplication', () => {
    ...
    beforeEach(() => {
        ...
        symfonyRouting.generate.mockImplementation((route: any, params: {
            [key: string]: string
        }) => {
            return `${route}?${Object.entries(params).map(([key, value]: [any, string]) => `${key}=${value}`).join('&')}`;
        });

        translate.mockImplementation((key: any) => key);
    });
