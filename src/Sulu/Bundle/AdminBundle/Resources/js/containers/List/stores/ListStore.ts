...
export default class ListStore {
    @observable
    pageCount: number | null | undefined = 0;
    @observable
    ...
    @observable
    deletingSelection: boolean = false;
    @observable
    ...
    @observable
    forbidden: boolean;
    active: IObservableValue<string | null | undefined | number> = observable.box();
    filterOptions: IObservableValue<{
        [key: string]: unknown
    }> = observable.box({});
    sortColumn: IObservableValue<string> = observable.box();
    sortOrder: IObservableValue<SortOrder> = observable.box();
    searchTerm: IObservableValue<string | null | undefined> = observable.box();
    limit: IObservableValue<number> = observable.box(10);
    resourceKey: string;
    listKey: string;
    userSettingsKey: string;
    observableOptions: ObservableOptions;
    localeDisposer: () => void | null | undefined;
    searchDisposer: () => void;
    filterDisposer: () => void;
    ...
    initialSelectionIds: Array<string | number> | null | undefined;
    pendingRequest: RequestPromise<any> | null | undefined;
    metadataOptions: any | null | undefined;
    ...
    static setActiveSetting(listKey: string, userSettingsKey: string, value: any) {
        const key = [USER_SETTING_PREFIX, listKey, userSettingsKey, USER_SETTING_ACTIVE].join('.');
    ...
    static setFilterSetting(listKey: string, userSettingsKey: string, value: any) {
        const key = [USER_SETTING_PREFIX, listKey, userSettingsKey, USER_SETTING_FILTER].join('.');
    ...
    static setSortColumnSetting(listKey: string, userSettingsKey: string, value: any) {
        const key = [USER_SETTING_PREFIX, listKey, userSettingsKey, USER_SETTING_SORT_COLUMN].join('.');
    ...
    static setSortOrderSetting(listKey: string, userSettingsKey: string, value: any) {
        const key = [USER_SETTING_PREFIX, listKey, userSettingsKey, USER_SETTING_SORT_ORDER].join('.');
    ...
    static setLimitSetting(listKey: string, userSettingsKey: string, value: any) {
        const key = [USER_SETTING_PREFIX, listKey, userSettingsKey, USER_SETTING_LIMIT].join('.');

        userStore.setPersistentSetting(key, value);
    }

    static getSchemaSetting(listKey: string, userSettingsKey: string): Array<any> {
        const key = [USER_SETTING_PREFIX, listKey, userSettingsKey, USER_SETTING_SCHEMA].join('.');

        return userStore.getPersistentSetting(key);
    }

    static setSchemaSetting(listKey: string, userSettingsKey: string, value: any) {
        const key = [USER_SETTING_PREFIX, listKey, userSettingsKey, USER_SETTING_SCHEMA].join('.');
        userStore.setPersistentSetting(key, value);
    }
    ...
    constructor(
        ...
        selectionIds?: Array<string | number> | null
    ) {
        ...
        const callResetForChangedObservable = (change: IValueWillChange<any>) => {
        ...
        if (locale) {
            this.localeDisposer = intercept(locale, '', (change: IValueWillChange<any>) => {
                callResetForChangedObservable(change);
                return change;
            });
        }

        this.searchDisposer = intercept(this.searchTerm, '', (change: IValueWillChange<any>) => {
            callResetForChangedObservable(change);
            return change;
        });

        this.filterDisposer = intercept(this.filterOptions, '', (change: IValueWillChange<any>) => {
            const oldValue = change.object.get();
            const oldFilteredValue = oldValue ?
                Object.keys(oldValue).reduce<Record<string, any>>((oldFilteredValue, currentKey) => {
            ...
            const newFilteredValue = newValue ?
                Object.keys(newValue).reduce<Record<string, any>>((newFilteredValue, currentKey) => {
        ...
        this.sortColumnDisposer = intercept(this.sortColumn, '', (change: IValueWillChange<any>) => {
            ListStore.setSortColumnSetting(this.listKey, this.userSettingsKey, change.newValue);
            callResetForChangedObservable(change);
            return change;
        });

        this.sortOrderDisposer = intercept(this.sortOrder, '', (change: IValueWillChange<any>) => {
            ListStore.setSortOrderSetting(this.listKey, this.userSettingsKey, change.newValue);
            callResetForChangedObservable(change);
            return change;
        });

        this.limitDisposer = intercept(this.limit, '', (change: IValueWillChange<any>) => {
            ListStore.setLimitSetting(this.listKey, this.userSettingsKey, change.newValue);
            callResetForChangedObservable(change);
            return change;
        });

        this.activeSettingDisposer = intercept(this.active, '', (change: IValueWillChange<any>) => {
    ...
    @computed
    get visibleItems(): Array<any> {
        return this.structureStrategy.visibleItems;
    }

    @computed
    get activeItems(): Array<any> | null | undefined {
        return this.structureStrategy.activeItems;
    }

    @computed
    get queryOptions(): any {
        const queryOptions = {...this.options} as const;
    ...
    @computed.struct get filterQueryOption() {
        ...
        return Object.keys(filterOptions).reduce<Record<string, any>>((filterQueryOption, column) => {
    ...
    @computed
    get filterableFields(): Schema | null | undefined {
        if (!this.schema) {
            return undefined;
        }

        return Object.keys(this.schema).reduce<Record<string, any>>((filterableFields, schemaKey) => {
    ...
    findById(id: string | number): any | null | undefined {
    ...
    copy = (id: string | number, parentId: string | number, callback?: ((response: any) => void) | null) => {
        const queryOptions = {
            ...this.options,
            action: 'copy',
            destination: parentId,
    ...
    sendRequest = () => {
        ...
        const observableOptions: Record<string, any> = {};
        ...
        const options = {...observableOptions, ...this.options} as const;
    ...
    @action setActive(active: string | null | undefined | number) {
        this.active.set(active);
    }

    @action activate(id: string | null | undefined | number) {
    ...
    @action deactivate(id: string | null | undefined | number) {
    ...
    @action search(searchTerm?: string | null) {
    ...
    @computed
    get selectionIds(): Array<string | number> {
        // TODO do not hardcode id but use metdata instead
