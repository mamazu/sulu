...
class StructureStrategy {
    ...
    remove = jest.fn();
    order = jest.fn();
    findById: (id: string | number) => any | null | undefined = jest.fn();
}
...
test('The loading strategy should be called with expandedIds if some items are already selected', () => {
    ...
    const promise = new RequestPromise(function(resolve: any) {
        resolve({});
    });
    loadingStrategy.load.mockReturnValue(promise);

    structureStrategy.findById.mockImplementation((id: any) => {
        switch (id) {
            case 1:
                return {id: 1};
            case 5:
...
test('Set loading flag to false after request', (done: any) => {
    const page = observable.box();
    const listStore = new ListStore('tests', 'tests', 'list_test', {page});
    listStore.schema = {};
    const promise = new RequestPromise(function(resolve: any) {
...
test('Set forbidden flag to true if the response returned a 403', (done: any) => {
    const listStore = new ListStore('tests', 'tests', 'list_test', {page: observable.box()});
    listStore.schema = {};

    const promise = new RequestPromise(function(resolve: any, reject: any) {
        reject({status: 403});
    });
...
test('Cancel request when a second one is started before finishing the first one', () => {
    ...
    loadingStrategy.load.mockReturnValueOnce(new RequestPromise(function(resolve: any){
        resolve();
    }));
    loadingStrategy.load.mockReturnValueOnce(new RequestPromise(function(resolve: any){
        resolve();
...
test('Set active to undefined if the response returned a 404', (done: any) => {
...
test('Should reload data but not change the page or the active item when the reload method is called', (done: any) => {
    const loadingStrategy = new LoadingStrategy();
    const structureStrategy = new StructureStrategy();
...
test('Should crash when deleting all selected items and one request fails with another error than 404', (done: any) => {
    const page = observable.box(1);
    const listStore = new ListStore('snippets', 'snippets', 'list_test', {page});
    listStore.schema = {};
    const structureStrategy = new StructureStrategy();
