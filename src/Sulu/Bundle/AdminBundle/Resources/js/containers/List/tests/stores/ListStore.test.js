...
const ResourceRequester_1 = __importStar(require("../../../../services/ResourceRequester"));
...
class LoadingStrategy {
    constructor() {
        ...
        this.load = jest.fn().mockReturnValue(new ResourceRequester_1.RequestPromise(function () { }));
        this.reset = jest.fn();
...
let StructureStrategy = (() => {
    var _a;
    let _data_decorators;
    let _data_initializers = [];
    let _data_extraInitializers = [];
    return _a = class StructureStrategy {
            constructor() {
                this.data = __runInitializers(this, _data_initializers, []);
                this.visibleItems = (__runInitializers(this, _data_extraInitializers), []);
...
test('The active item should be updated when set from the outside', () => {
    const listStore = new ListStore_1.default('tests', 'tests', 'list_test', { page: mobx_1.observable.box() });
    expect(listStore.active.get()).toEqual();
    listStore.active.set('123');
    expect(stores_1.userStore.setPersistentSetting).toBeCalledWith('sulu_admin.list_store.tests.list_test.active', '123');
});
test('The filter value should be updated when set from the outside', () => {
    ...
    expect(stores_1.userStore.setPersistentSetting)
...
test('The limit value should be updated when set from the outside', () => {
    ...
    expect(stores_1.userStore.setPersistentSetting).toBeCalledWith('sulu_admin.list_store.tests.list_test.limit', 20);
});
test('The sort column value should be updated when set from the outside', () => {
    ...
    expect(stores_1.userStore.setPersistentSetting).toBeCalledWith('sulu_admin.list_store.tests.list_test.sort_column', 'title');
});
test('The sort order value should be updated when set from the outside', () => {
    ...
    expect(stores_1.userStore.setPersistentSetting).toBeCalledWith('sulu_admin.list_store.tests.list_test.sort_order', 'asc');
});
...
test('The loading strategy should be called when a request is sent', () => {
    const loadingStrategy = new LoadingStrategy();
    const structureStrategy = new StructureStrategy();
    const page = mobx_1.observable.box(1);
    const locale = mobx_1.observable.box();
    const additionalValue = mobx_1.observable.box(5);
    const listStore = new ListStore_1.default('tests', 'tests', 'list_test', {
        page,
        locale,
        additionalValue,
    }, {
    ...
    listStore.updateLoadingStrategy(loadingStrategy);
    listStore.updateStructureStrategy(structureStrategy);
    expect(loadingStrategy.load).toBeCalledWith('tests', {
        additionalValue: 5,
        fields: [
            'id',
        ],
...
test('The user store should be called correctly when changing the schema', () => {
    ...
    metadataStore_1.default.getSchema.mockReturnValueOnce(schemaPromise);
    const listStore = new ListStore_1.default('tests', 'tests', 'list_test', {
        page,
        locale,
        additionalValue,
    ...
    expect((metadataStore_1.default).getSchema).toBeCalledWith('tests', { id: 1 });
    return schemaPromise.then(() => {
        ...
        expect(stores_1.userStore.setPersistentSetting).toBeCalledWith('sulu_admin.list_store.tests.list_test.schema', [
            {
                'schemaKey': 'id',
                'visibility': 'no',
            },
...
test('The userSchema should include schema properties that are not present in the schemaSetting of the user', () => {
    ...
    stores_1.userStore.getPersistentSetting.mockReturnValueOnce(schemaSetting);
    const listStore = new ListStore_1.default('tests', 'tests', 'list_test', {
        page,
    }, {
        test: 'value',
    ...
    expect((stores_1.userStore).getPersistentSetting).toBeCalledWith('sulu_admin.list_store.tests.list_test.schema');
    listStore.destroy();
});
...
test('The loading strategy should be called with a different resourceKey when a request is sent', () => {
    ...
    const page = mobx_1.observable.box(1);
    const locale = mobx_1.observable.box();
    const listStore = new ListStore_1.default('snippets', 'snippets', 'list_test', {
        page,
        locale,
    }, {
        test: 'value',
    ...
    listStore.updateLoadingStrategy(loadingStrategy);
    listStore.updateStructureStrategy(structureStrategy);
    expect(loadingStrategy.load).toBeCalledWith('snippets', {
        fields: [
            'id',
        ],
        locale: undefined,
...
test('The loading strategy should be called with the defined search', () => {
    const loadingStrategy = new LoadingStrategy();
    const structureStrategy = new StructureStrategy();
    const page = mobx_1.observable.box(2);
    const listStore = new ListStore_1.default('snippets', 'snippets', 'list_test', {
        page,
    });
    listStore.schema = {};
...
test('The loading strategy should be called with expandedIds if some items are already selected', () => {
    ...
    const structureStrategy = new StructureStrategy();
    const page = mobx_1.observable.box(1);
    const listStore = new ListStore_1.default('categories', 'categories', 'list_test', {
        page,
    }, {}, {}, [1, 5, 10]);
    listStore.schema = {};
    ...
    listStore.updateLoadingStrategy(loadingStrategy);
    listStore.updateStructureStrategy(structureStrategy);
    expect(loadingStrategy.load).toBeCalledWith('categories', {
        fields: [
            'id',
        ],
        selectedIds: '1,5,10',
...
test('Set loading flag to true before schema is loaded', () => {
    ...
    metadataStore_1.default.getSchema.mockReturnValueOnce(promise);
    const page = mobx_1.observable.box();
    const listStore = new ListStore_1.default('tests', 'tests', 'list_test', { page });
...
test('Set forbidden flag to true if the response returned a 403', (done) => {
    const listStore = new ListStore_1.default('tests', 'tests', 'list_test', { page: mobx_1.observable.box() });
    listStore.schema = {};
    const promise = new ResourceRequester_1.RequestPromise(function (resolve, reject) {
        reject({ status: 403 });
    });
    const loadingStrategy = new LoadingStrategy();
    loadingStrategy.load.mockReturnValue(promise);
...
test('Cancel request when a second one is started before finishing the first one', () => {
    ...
    const loadingStrategy = new LoadingStrategy();
    const structureStrategy = new StructureStrategy();
    loadingStrategy.load.mockReturnValueOnce(new ResourceRequester_1.RequestPromise(function (resolve) {
        resolve();
    }));
    loadingStrategy.load.mockReturnValueOnce(new ResourceRequester_1.RequestPromise(function (resolve) {
        resolve();
    ...
    listStore.updateLoadingStrategy(loadingStrategy);
    listStore.updateStructureStrategy(structureStrategy);
    expect(loadingStrategy.load).toHaveBeenLastCalledWith('tests', { fields: ['id'], limit: 10, page: 1, sortBy: undefined, sortOrder: undefined }, undefined);
...
test('Set active to undefined if the response returned a 404', (done) => {
    const listStore = new ListStore_1.default('tests', 'tests', 'list_test', { page: mobx_1.observable.box() }, undefined);
    listStore.schema = {};
    const promise = new ResourceRequester_1.RequestPromise(function (resolve, reject) {
        reject({ status: 404 });
    });
    ...
    setTimeout(() => {
        ...
        expect(stores_1.userStore.setPersistentSetting).toBeCalledWith('sulu_admin.list_store.tests.list_test.active', undefined);
        done();
    });
});
test('Get schema from MetadataStore for correct resourceKey', () => {
    ...
    listStore.updateLoadingStrategy(new LoadingStrategy());
    listStore.updateStructureStrategy(new StructureStrategy());
    expect(metadataStore_1.default.getSchema).toBeCalledWith('tests', undefined);
    return schemaPromise.then(() => {
...
test('Select an item', () => {
    ...
    expect((0, mobx_1.toJS)(listStore.selectionIds)).toEqual([1, 2]);
    listStore.deselect({ id: 1 });
    expect((0, mobx_1.toJS)(listStore.selectionIds)).toEqual([2]);
    listStore.destroy();
});
test('Deselect an item that has not been selected yet', () => {
    ...
    expect((0, mobx_1.toJS)(listStore.selectionIds)).toEqual([1]);
    listStore.destroy();
});
test('Select all visible items', () => {
    ...
    expect((0, mobx_1.toJS)(listStore.selectionIds)).toEqual([1, 7, 2, 3]);
    expect(loglevel_1.default.warn).toBeCalledWith(expect.stringContaining('The "selectVisibleItems" method'));
    listStore.destroy();
});
test('Deselect all visible items', () => {
    ...
    expect((0, mobx_1.toJS)(listStore.selectionIds)).toEqual([7]);
    expect(loglevel_1.default.warn).toBeCalledWith(expect.stringContaining('The "deselectVisibleItems" method'));
    listStore.destroy();
});
...
test('Should reload data but not change the page or the active item when the reload method is called', (done) => {
    ...
    expect(structureStrategy.findById).toBeCalledWith(1);
    (0, mobx_1.when)(() => !listStore.loading, () => {
...
test('Should reset page count to 0 and page to 1 when locale is changed', () => {
    const page = mobx_1.observable.box(3);
    const locale = mobx_1.observable.box('en');
    const listStore = new ListStore_1.default('snippets', 'snippets', 'list_test', { page, locale });
    listStore.schema = {};
    const loadingStrategy = new LoadingStrategy();
    const structureStrategy = new StructureStrategy();
    listStore.updateLoadingStrategy(loadingStrategy);
...
test('Should reset page count and page when loading strategy changes', () => {
    const page = mobx_1.observable.box();
    const listStore = new ListStore_1.default('snippets', 'snippets', 'list_test', { page });
    listStore.schema = {};
    const loadingStrategy = new LoadingStrategy();
    const structureStrategy = new StructureStrategy();
...
test('Should clear the StructureStrategy when the clear method is called', () => {
    const listStore = new ListStore_1.default('snippets', 'snippets', 'list_test', { page: mobx_1.observable.box() });
    listStore.schema = {};
    const structureStrategy = new StructureStrategy();
    listStore.clear();
...
test('Should trigger a mobx autorun if activate is called with the same id', () => {
    ...
    let lastActive;
    const autorunDisposer = (0, mobx_1.autorun)(() => {
...
test('Should move the item with the given ID to the new given parent and reload the list', () => {
    ...
    const listStore = new ListStore_1.default('snippets', 'snippets', 'list_test', { page: mobx_1.observable.box(), locale }, { webspace: 'sulu' });
    return schemaPromise.then(() => {
        const loadingStrategy = new LoadingStrategy();
        const structureStrategy = new StructureStrategy();
        listStore.updateLoadingStrategy(loadingStrategy);
        ...
        ResourceRequester_1.default.post.mockReturnValue(postWithIdPromise);
        listStore.move(3, 8);
        expect(ResourceRequester_1.default.post).toBeCalledWith('snippets', undefined, { action: 'move', destination: 8, id: 3, locale: 'de', webspace: 'sulu' });
        return postWithIdPromise.then(() => {
            expect(structureStrategy.clear).toBeCalledWith();
            expect(loadingStrategy.load).toHaveBeenLastCalledWith('snippets', {
                fields: [
                    'title',
                    'id',
                ],
...
test('Should move all selected items to the new given parent and reload the list', () => {
    ...
    return schemaPromise.then(() => {
        ...
        expect(ResourceRequester_1.default.post).toBeCalledWith('snippets', undefined, { action: 'move', destination: 3, id: 1, locale: 'de', webspace: 'sulu' });
        expect(ResourceRequester_1.default.post).toBeCalledWith('snippets', undefined, { action: 'move', destination: 3, id: 2, locale: 'de', webspace: 'sulu' });
        expect(ResourceRequester_1.default.post).toBeCalledWith('snippets', undefined, { action: 'move', destination: 3, id: 4, locale: 'de', webspace: 'sulu' });
...
test('Should copy the item with the given ID to the new given parent and reload the list', () => {
    ...
    return schemaPromise.then(() => {
        ...
        expect(ResourceRequester_1.default.post).toBeCalledWith('snippets', undefined, { action: 'copy', destination: 8, id: 3, locale: 'de', webspace: 'sulu' });
...
test('Should delete the item with the given ID and options', () => {
    const page = mobx_1.observable.box(1);
    const locale = mobx_1.observable.box('en');
    const listStore = new ListStore_1.default('snippets', 'snippets', 'list_test', { page, locale }, { webspace: 'sulu' });
    listStore.schema = {};
    const deletePromise = Promise.resolve();
    ResourceRequester_1.default.delete.mockReturnValue(deletePromise);
    const loadingStrategy = new LoadingStrategy();
    const structureStrategy = new StructureStrategy();
    listStore.updateLoadingStrategy(loadingStrategy);
    listStore.updateStructureStrategy(structureStrategy);
    listStore.delete(5, { force: true });
    expect(ResourceRequester_1.default.delete).toBeCalledWith('snippets', { force: true, id: 5, locale: 'en', webspace: 'sulu' });
...
test('Should delete the item with the given ID and remove it from the selection afterwards', () => {
    ...
    expect((0, mobx_1.toJS)(listStore.selections)).toEqual([{ id: 5 }]);
    listStore.delete(5);
    expect(ResourceRequester_1.default.delete).toBeCalledWith('snippets', { id: 5, locale: 'en' });
    return deletePromise.then(() => {
        expect((0, mobx_1.toJS)(listStore.selections)).toEqual([]);
        expect(structureStrategy.remove).toBeCalledWith(5);
    });
});
test('Should delete the item with the given ID without locale', () => {
    const page = mobx_1.observable.box(1);
    const listStore = new ListStore_1.default('snippets', 'snippets', 'list_test', { page }, { webspace: 'sulu' });
    listStore.schema = {};
    const deletePromise = Promise.resolve();
    ResourceRequester_1.default.delete.mockReturnValue(deletePromise);
    const loadingStrategy = new LoadingStrategy();
    ...
    listStore.updateStructureStrategy(structureStrategy);
    listStore.delete(5);
    expect(ResourceRequester_1.default.delete).toBeCalledWith('snippets', { id: 5, webspace: 'sulu' });
    return deletePromise.then(() => {
        expect(structureStrategy.remove).toBeCalledWith(5);
    });
});
test('Should delete all selected items and reload the list afterwards', () => {
    ...
    metadataStore_1.default.getSchema.mockReturnValueOnce(schemaPromise);
    ResourceRequester_1.default.delete.mockReturnValue(Promise.resolve());
    const page = mobx_1.observable.box(1);
    const listStore = new ListStore_1.default('snippets', 'snippets', 'list_test', { page });
    const loadingStrategy = new LoadingStrategy();
    const structureStrategy = new StructureStrategy();
    ...
    return schemaPromise.then(() => {
        ...
        expect(loadingStrategy.load).toBeCalledTimes(1);
        return deletePromise.then(() => {
            expect(ResourceRequester_1.default.delete).toHaveBeenCalledTimes(2);
            expect(ResourceRequester_1.default.delete).toBeCalledWith('snippets', { id: 1 });
            expect(ResourceRequester_1.default.delete).toBeCalledWith('snippets', { id: 2 });
...
test('Should delete all selected items and succeed even if one of them returns a 404', () => {
    ...
    const structureStrategy = new StructureStrategy();
    listStore.updateStructureStrategy(structureStrategy);
    ResourceRequester_1.default.delete.mockReturnValue(Promise.reject({ status: 404 }));
    listStore.select({ id: 1 });
    listStore.select({ id: 2 });
    const deletePromise = listStore.deleteSelection();
    return deletePromise.then(() => {
...
test('Should crash when deleting all selected items and one request fails with another error than 404', (done) => {
    ...
    const structureStrategy = new StructureStrategy();
    listStore.updateStructureStrategy(structureStrategy);
    ResourceRequester_1.default.delete.mockReturnValue(Promise.reject({ status: 500 }));
    listStore.select({ id: 1 });
    listStore.select({ id: 2 });
    expect(listStore.deletingSelection).toEqual(false);
    const deletePromise = listStore.deleteSelection();
...
test('Should order the item with the given ID and options to the given position', () => {
    ...
    ResourceRequester_1.default.post.mockReturnValue(orderPromise);
    const loadingStrategy = new LoadingStrategy();
    const structureStrategy = new StructureStrategy();
    listStore.updateLoadingStrategy(loadingStrategy);
    listStore.updateStructureStrategy(structureStrategy);
    listStore.order(5, 1);
    expect(ResourceRequester_1.default.post)
