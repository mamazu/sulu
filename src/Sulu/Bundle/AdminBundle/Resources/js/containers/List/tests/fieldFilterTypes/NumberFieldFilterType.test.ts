...
test.each([
    ...
    [{lt: 2}],
    [{gt: 7}],
])('Render with value of "%s"', (value: any) => {
...
test.each([
    ...
    [{lt: 9}, 'gt', {gt: 9}],
    [{gt: 3}, 'eq', {eq: 3}],
])('Call onChange handler when the value was "%s" and the "%s" operator is chosen', (value: any, operator: any, newValue: any) => {
    const changeSpy = jest.fn();
...
test.each([
    ...
    [{lt: 9}, 4, {lt: 4}],
    [{gt: 3}, 9, {gt: 9}],
])('Call onChange handler when the value was "%s" and the number was changed to "%s"', (value: any, number: any, newValue: any) => {
    const changeSpy = jest.fn();
    const numberFieldFilterType = new NumberFieldFilterType(changeSpy, {}, value);
    const numberFieldFilterTypeForm = mount(numberFieldFilterType.getFormNode());
...
test.each([
    ...
    [{gt: 9}, '> 9'],
    [undefined, ' '],
])('Return value node with value "%s"', (value: any, expectedValueNode: any) => {
