...
let mockStructureStrategyData: any;
let mockStructureStrategyVisibleItems: any;
...
test('Render the adapter with filters', () => {
    ...
    listFieldFilterTypeRegistry.get.mockImplementation((key: any) => {
        switch (key) {
            case 'datetime':
            case 'text':
                return class {
...
test('Pass adapterOptions to the adapter', () => {
    const adapterOptions = {table: {show_header: true}, test: {skin: 'light'}} as const;
    const listStore = new ListStore('test', 'test', 'list_test', {page: observable.box(1)});
...
test('Pass undefined as adapterOptions to the adapter if no options for current adapter are passed', () => {
    const adapterOptions = {table: {skin: 'flat'}} as const;
    const listStore = new ListStore('test', 'test', 'list_test', {page: observable.box(1)});
    const list = shallow(<List adapterOptions={adapterOptions} adapters={['test']} store={listStore} />);
...
test('Pass options to adapter', () => {
    ...
    const listAdapterOptions = {test: 'value'} as const;
...
test('Should start with adapter from user settings', () => {
    ...
    listAdapterRegistry.get.mockImplementation((adapter: any) => {
        switch (adapter) {
            case 'table':
                return TableAdapter;
            case 'folder':
...
test('Correct LoadingStrategyOptions should be passed to the LoadingStrategy if paginated prop is set', () => {
    ...
    TableAdapter.LoadingStrategy = (jest.fn() as any);
...
test('Correct LoadingStrategyOptions should not be passed to the LoadingStrategy if adapter is not paginatable', () => {
    ...
    ColumnListAdapter.LoadingStrategy = (jest.fn() as any);
...
test('ListStore should delete linked item when onRequestItemDelete callback is is confirmed twice', (done: any) => {
...
test('ListStore should not delete linked item when onRequestItemDelete callback is is confirmed once', (done: any) => {
    const jsonDeletePromise = Promise.resolve({
        code: 1106,
        resource: {
            id: 5,
...
test('ListStore should delete linked item when called with allowConflictDeletion value of true', (done: any) => {
    const jsonDeletePromise = Promise.resolve({
        code: 1106,
        resource: {
            id: 5,
...
test('ListStore should not delete linked item when called with allowConflictDeletion value of false', (done: any) => {
    const jsonDeletePromise = Promise.resolve({
        code: 1106,
        resource: {
            id: 5,
...
test('ListStore should delete item with dependants when onFinish callback called', (done: any) => {
    const jsonDeletePromise = Promise.resolve({
        code: 1105,
        resource: {
            id: 5,
...
test('ListStore should not delete item with dependants when onCancel callback called', (done: any) => {
    const jsonDeletePromise = Promise.resolve({
        code: 1105,
        resource: {
            id: 5,
