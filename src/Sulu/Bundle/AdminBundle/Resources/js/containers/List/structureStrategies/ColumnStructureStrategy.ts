...
export default class ColumnStructureStrategy implements StructureStrategyInterface {
    @observable
    rawData: Map<string | null | undefined | number, Array<ColumnItem>> = new Map();

    @computed
    get visibleItems(): Array<ColumnItem> {
        return this.data.reduce<Array<any>>((data, items) => data.concat(...items), []);
    }

    @computed
    get activeItems(): Array<string | null | undefined | number> {
        return Array.from(this.rawData.keys());
    }

    @computed
    get data(): Array<Array<ColumnItem>> {
    ...
    activate(id: string | null | undefined | number) {
    ...
    findById(identifier: string | number): ColumnItem | null | undefined {
    ...
    addItem(item: any, parentId: string | null | undefined | number) {
