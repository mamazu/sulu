...
test.each([
    ...
    const selectFieldFilterType = new SelectFieldFilterType_1.default(jest.fn(), parameters, undefined);
...
test.each([
    ...
    const selectFieldFilterType = new SelectFieldFilterType_1.default(jest.fn(), parameters, value);
    expect((0, enzyme_1.render)(selectFieldFilterType.getFormNode())).toMatchSnapshot();
});
test('Render with value set by setValue', () => {
    const selectFieldFilterType = new SelectFieldFilterType_1.default(jest.fn(), { options: { audio: 'sulu_media.audio' } }, undefined);
    selectFieldFilterType.setValue(['audio']);
    expect((0, enzyme_1.render)(selectFieldFilterType.getFormNode())).toMatchSnapshot();
});
test('Pass correct props to CheckboxGroup', () => {
    const selectFieldFilterType = new SelectFieldFilterType_1.default(jest.fn(), { options: { audio: 'Audio', image: 'Image', video: 'Video' } }, ['audio', 'video']);
    const selectFieldFilterTypeForm = (0, enzyme_1.mount)(selectFieldFilterType.getFormNode());
...
test('Call onChange handler with new value', () => {
    ...
    const selectFieldFilterType = new SelectFieldFilterType_1.default(changeSpy, { options: { test: 'test' } }, undefined);
    const selectFieldFilterTypeForm = (0, enzyme_1.mount)(selectFieldFilterType.getFormNode());
...
test.each([
    ...
    const selectFieldFilterType = new SelectFieldFilterType_1.default(jest.fn(), { options: { audio: 'Audio', image: 'Image', video: 'Video' } }, undefined);
