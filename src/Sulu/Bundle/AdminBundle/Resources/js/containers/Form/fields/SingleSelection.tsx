...
class SingleSelection extends React.Component<Props> {
    ...
    autoCompleteSelectionStore: SingleSelectionStore<string | number> | null | undefined;
    changeAutoCompleteSelectionDisposer: () => any | null | undefined;
    ...
    handleAutoCompleteSelectionChange = (selectedItem?: {
        id: Value
    } | null) => {
        ...
        if (this.value !== selectedItem?.id) {
            if (this.useDeprecatedObjectDataFormat) {
                this.handleChange((selectedItem as any));
            } else {
    ...
    handleItemClick = (itemId: Value, item?: any | null) => {
        ...
        router.navigate(
            viewName,
            Object.keys(resultToView).reduce<Record<string, any>>((parameters, resultPath) => {
    ...
    renderListOverlay() {
        ...
        const formListOptions = formOptionsToListOptions.reduce<Record<string, any>>((currentOptions, formOption) => {
            if (!formOption.name) {
    ...
    renderAutoComplete() {
        ...
        const options = {
            ...dataPathToAutoComplete.reduce<Record<string, any>>((options, schemaEntry) => {
