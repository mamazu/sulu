...
export default class ResourceFormStore extends AbstractFormStore implements FormStoreInterface {
    ...
    schemaDisposer: () => void | null | undefined;
    metadataOptions: {
        [key: string]: any
    } | null | undefined;

    constructor(resourceStore: ResourceStore, formKey: string, options: any = {}, metadataOptions?: any | null) {
        super();
    ...
    @action handleSchemaTypeResponse = (schemaTypes?: SchemaTypes | null) => {
        const {
            types = {},
            defaultType,
        } = schemaTypes || {};
    ...
    handleSchemaResponse = ([schema, jsonSchema]: [any, any]) => {
    ...
    loadAndMergeRemoteData = (localSchema: Schema, remoteSchema: Schema) => {
        ...
        if (localSchema) {
            return this.resourceStore.requestRemoteData({template: this.type}).then((data: any) => {
    ...
    copyFromLocale(sourceLocale: string, options: any) {
    ...
    change(dataPath: string, value: unknown, context?: ChangeContext) {
