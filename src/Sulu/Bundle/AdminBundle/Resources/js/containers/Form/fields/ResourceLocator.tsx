...
@observer
class ResourceLocator extends React.Component<FieldTypeProps<string | null | undefined>> {
    @observable
    mode: string;
    @observable
    inputChanged: boolean = false;
    @observable
    inputChangedSinceRefresh: boolean = false;
    @observable
    partsChangedSinceRefresh: boolean = false;

    partsChangeDisposer: () => unknown | null | undefined;
    ...
    get parts(): {
        ...
        const partEntries = formInspector.getPathsByTag(PART_TAG)
            ...
            .filter(([, value]: [any, unknown]) => !!value)
    ...
    @computed
    get enableAutoGeneration(): boolean {
        const {
            formInspector: {
                id,
            },
    ...
    constructor(props: FieldTypeProps<string | null | undefined>) {
        super(props);
    ...
    @action refreshResourceLocator = () => {
        ...
        const requestOptions = {...formInspector.options} as const;

        Object.entries(resourceStorePropertiesToRequest).forEach(([propertyName, parameterName]: [any, any]) => {
    ...
    @action handleInputChange = (value?: string | null) => {
