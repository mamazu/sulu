...
test('Return form and schema fields for given resourceKey from MetadataStore', () => {
    ...
    generalMetadataStore.loadMetadata.mockImplementation((type: any, resourceKey: any) => {
        switch (resourceKey) {
            case 'snippets':
    ...
    ).then(([snippetFields, contactFields, snippetSchema, contactSchema]: [any, any, any, any]) => {
...
test('Return form fields for given resourceKey and type', () => {
    ...
    ]).then(([snippetSidebarFields, snippetFooterFields]: [any, any]) => {
...
test('Return schema for given resourceKey and type', () => {
    ...
    ]).then(([snippetSidebar, snippetFooter]: [any, any]) => {
        expect(snippetSidebar).toBe(snippetSidebarSchema);
        expect(snippetFooter).toBe(snippetFooterSchema);
    });
});

test('Throw if a type is requested, but the given resourceKey does not have type support', (done: any) => {
    const snippetMetadata = {
        form: {
            title: {},
            description: {},
    ...
    return snippetFieldsPromise.catch((error: any) => {
...
test('Throw if a schema for a type is requested, but the given resourceKey does not have type support', (done: any) => {
    const snippetMetadata = {
        form: {
            title: {},
            description: {},
...
test('Throw if a type is omitted, but the given reosurceKey has type support', (done: any) => {
    const snippetMetadata = {
        types: {
            sidebar: {
                schema: {
    ...
    return snippetSchemaPromise.catch((error: any) => {
        expect(generalMetadataStore.loadMetadata).toBeCalledWith('form', 'snippets', undefined);
...
test('Throw if a type is omitted when loading the JSON Schema, but the given reosurceKey has type support', (done: any) => {
    const snippetMetadata = {
        types: {
            sidebar: {
                schema: {
...
test('Throw exception if no form fields for given resourceKey are available', () => {
    ...
    return metadataStore.getSchema('contacts').catch((error: any) => {
...
test('Throw exception if no schema for given resourceKey are available', () => {
    ...
    return metadataStore.getJsonSchema('contacts').catch((error: any) => {
        expect(generalMetadataStore.loadMetadata).toBeCalledWith('form', 'contacts', undefined);
        expect(error.toString()).toEqual(expect.stringContaining('"contacts"'));
    });
});
...
test('Throw exception if no form fields for given resourceKey and type are available', () => {
    ...
    return metadataStore.getSchema('snippets', 'default').catch((error: any) => {
        expect(generalMetadataStore.loadMetadata).toBeCalledWith('form', 'snippets', undefined);
...
test('Throw exception if no form fields for given resourceKey and type are available', () => {
    ...
    return metadataStore.getJsonSchema('snippets', 'default').catch((error: any) => {
        expect(generalMetadataStore.loadMetadata).toBeCalledWith('form', 'snippets', undefined);
...
test('Return available types with default type for given resourceKey', () => {
    ...
    return snippetTypesPromise.then((snippetTypes: any) => {
