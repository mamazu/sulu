...
jest.mock('../../../../stores/ResourceStore', () => (function(resourceKey: any, id: any, options: any) {
    this.id = id;
    ...
    this.set = jest.fn(function(path: any, value: any) {
        this.data[path] = value;
    });
    this.setMultiple = jest.fn(function(data: any) {
        this.data = {...this.data, ...data};
    });
    this.change = jest.fn(function(path: any, value: any) {
        this.data[path] = value;
    });
    this.remove = jest.fn();
    this.changeMultiple = jest.fn(function(data: any) {
        this.data = {...this.data, ...data};
    });
...
test('Create data object for schema', (done: any) => {
    const metadata = {
        title: {
            label: 'Title',
            type: 'text_line',
...
test('Change schema should keep data', (done: any) => {
    const metadata = {
        title: {
            label: 'Title',
            type: 'text_line',
...
test('Change type should update schema and data', (done: any) => {
...
test('Change schema should update data and use remoteData for invalid blocks', (done: any) => {
    const oldSchema = {
        title: {
            label: 'Title',
            type: 'text_line',
...
test('Change schema should update data and use default-type for unknown block types', (done: any) => {
    const oldSchema = {
        title: {
            label: 'Title',
            type: 'text_line',
...
test('Change schema should merge locale and remote data', (done: any) => {
    const newSchema = {
        title: {
            label: 'Title',
            type: 'text_line',
...
test('Change schema should merge current and origin data partially in block', (done: any) => {
    const oldSchema = {
        title: {
            label: 'Title',
            type: 'text_line',
...
test('Change schema should merge current and origin data partially block in blocks', (done: any) => {
    const oldSchema = {
        title: {
            label: 'Title',
            type: 'text_line',
...
test.each([true, false])('Forbidden flag should be set as %s', (forbidden: any) => {
...
test('Save the store should reject if request has failed', (done: any) => {
    const jsonSchemaPromise = Promise.resolve({});
    metadataStore.getJsonSchema.mockReturnValue(jsonSchemaPromise);
...
test('Validate should return true if no errors occured', (done: any) => {
    const jsonSchemaPromise = Promise.resolve({
        type: 'object',
        required: ['title'],
    });
...
test('Validate should return false if errors occured', (done: any) => {
    const jsonSchemaPromise = Promise.resolve({
        type: 'object',
        required: ['title'],
    });
...
test('Validate should not return incorrect errors', (done: any) => {
...
test('Save the store should validate the current data and an oneOf', (done: any) => {
    const jsonSchemaPromise = Promise.resolve({
        type: 'object',
...
test('Save the store should validate the current data and an anyOf', (done: any) => {
    const jsonSchemaPromise = Promise.resolve({
        type: 'object',
        required: ['title', 'blocks'],
        properties: {
...
test('Save the store should validate the current data and skip type errors', (done: any) => {
    const jsonSchemaPromise = Promise.resolve({
        type: 'object',
        properties: {
            images: {
...
test('Return SchemaEntry for given schemaPath', (done: any) => {
    const metadataPromise = Promise.resolve(
        {
            title: {
                tags: [
...
test.each(['sidebar', 'footer'])('Set type to default "%s" if loaded data has no template set', (defaultType: any, done: any) => {
    const schemaTypesPromise = Promise.resolve({
        types: {
            sidebar: {key: 'sidebar', title: 'Sidebar'},
            footer: {key: 'footer', title: 'Footer'},
