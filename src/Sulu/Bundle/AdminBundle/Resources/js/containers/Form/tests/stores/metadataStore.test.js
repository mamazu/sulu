...
test('Return form and schema fields for given resourceKey from MetadataStore', () => {
    ...
    metadataStore_2.default.loadMetadata.mockImplementation((type, resourceKey) => {
        switch (resourceKey) {
            case 'snippets':
    ...
    const snippetFieldPromise = metadataStore_1.default.getSchema('snippets');
    const contactFieldPromise = metadataStore_1.default.getSchema('contacts');
    const snippetSchemaPromise = metadataStore_1.default.getJsonSchema('snippets');
    const contactSchemaPromise = metadataStore_1.default.getJsonSchema('contacts');
    return Promise.all([
        snippetFieldPromise,
        contactFieldPromise,
...
test('Return form fields for given resourceKey and type', () => {
    ...
    const snippetPromise = Promise.resolve(snippetMetadata);
    metadataStore_2.default.loadMetadata.mockReturnValue(snippetPromise);
    const snippetSidebarFieldPromise = metadataStore_1.default.getSchema('snippets', 'sidebar');
    const snippetFooterFieldPromise = metadataStore_1.default.getSchema('snippets', 'footer');
    return Promise.all([
...
test('Return schema for given resourceKey and type', () => {
    ...
    const snippetPromise = Promise.resolve(snippetMetadata);
    metadataStore_2.default.loadMetadata.mockReturnValue(snippetPromise);
    const snippetSidebarSchemaPromise = metadataStore_1.default.getJsonSchema('snippets', 'sidebar');
    const snippetFooterSchemaPromise = metadataStore_1.default.getJsonSchema('snippets', 'footer');
    return Promise.all([
...
test('Throw if a type is requested, but the given resourceKey does not have type support', (done) => {
    ...
    const snippetPromise = Promise.resolve(snippetMetadata);
    metadataStore_2.default.loadMetadata.mockReturnValue(snippetPromise);
    const snippetFieldsPromise = metadataStore_1.default.getSchema('snippets', 'sidebar');
    return snippetFieldsPromise.catch((error) => {
        expect(metadataStore_2.default.loadMetadata).toBeCalledWith('form', 'snippets', undefined);
...
test('Throw if a schema for a type is requested, but the given resourceKey does not have type support', (done) => {
    ...
    const snippetPromise = Promise.resolve(snippetMetadata);
    metadataStore_2.default.loadMetadata.mockReturnValue(snippetPromise);
    const snippetFieldsPromise = metadataStore_1.default.getJsonSchema('snippets', 'sidebar', undefined);
    return snippetFieldsPromise.catch((error) => {
        expect(metadataStore_2.default.loadMetadata).toBeCalledWith('form', 'snippets', undefined);
        expect(error.toString()).toEqual(expect.stringContaining('does not support types'));
        expect(error.toString()).toEqual(expect.stringContaining('"snippets"'));
...
test('Throw if a type is omitted, but the given reosurceKey has type support', (done) => {
    ...
    const snippetPromise = Promise.resolve(snippetMetadata);
    metadataStore_2.default.loadMetadata.mockReturnValue(snippetPromise);
    const snippetSchemaPromise = metadataStore_1.default.getSchema('snippets');
...
test('Throw exception if no form fields for given resourceKey are available', () => {
    const contactMetadata = {};
    const contactPromise = Promise.resolve(contactMetadata);
    metadataStore_2.default.loadMetadata.mockReturnValue(contactPromise);
    return metadataStore_1.default.getSchema('contacts').catch((error) => {
        expect(metadataStore_2.default.loadMetadata).toBeCalledWith('form', 'contacts', undefined);
...
test('Throw exception if no schema for given resourceKey are available', () => {
    ...
    const contactPromise = Promise.resolve(contactMetadata);
    metadataStore_2.default.loadMetadata.mockReturnValue(contactPromise);
    return metadataStore_1.default.getJsonSchema('contacts').catch((error) => {
        expect(metadataStore_2.default.loadMetadata).toBeCalledWith('form', 'contacts', undefined);
        expect(error.toString()).toEqual(expect.stringContaining('"contacts"'));
    });
});
test('Throw exception if no form fields for given resourceKey and type are available', () => {
    ...
    const snippetPromise = Promise.resolve(snippetMetadata);
    metadataStore_2.default.loadMetadata.mockReturnValue(snippetPromise);
    return metadataStore_1.default.getSchema('snippets', 'default').catch((error) => {
        expect(metadataStore_2.default.loadMetadata).toBeCalledWith('form', 'snippets', undefined);
...
test('Throw exception if no form fields for given resourceKey and type are available', () => {
    ...
    const snippetPromise = Promise.resolve(snippetMetadata);
    metadataStore_2.default.loadMetadata.mockReturnValue(snippetPromise);
    return metadataStore_1.default.getJsonSchema('snippets', 'default').catch((error) => {
        expect(metadataStore_2.default.loadMetadata).toBeCalledWith('form', 'snippets', undefined);
...
test('Return available types with default type for given resourceKey', () => {
    ...
    const snippetPromise = Promise.resolve(snippetMetadata);
    metadataStore_2.default.loadMetadata.mockReturnValue(snippetPromise);
    const snippetTypesPromise = metadataStore_1.default.getSchemaTypes('snippets');
    expect(metadataStore_2.default.loadMetadata).toBeCalledWith('form', 'snippets', undefined);
