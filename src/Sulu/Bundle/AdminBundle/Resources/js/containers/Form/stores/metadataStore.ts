...
class MetadataStore {
    getSchemaTypes(formKey: string, metadataOptions?: any | null): Promise<SchemaTypes | null | undefined> {
        return metadataStore.loadMetadata(FORM_TYPE, formKey, metadataOptions)
            .then((configuration) => {
                ...
                return {
                    defaultType,
                    types: Object.keys(types).reduce<Record<string, any>>((transformedTypes, key) => {
    ...
    getSchema(formKey: string, type?: string | null, metadataOptions?: any | null): Promise<Schema> {
        return metadataStore.loadMetadata(FORM_TYPE, formKey, metadataOptions)
            .then((configuration) => {
    ...
    enhanceBlockForm(form: any, blockSchema: any) {
    ...
    getJsonSchema(formKey: string, type?: string | null, metadataOptions?: any | null): Promise<any> {
        return metadataStore.loadMetadata(FORM_TYPE, formKey, metadataOptions)
            .then((configuration) => {
                const typeConfiguration = this.getTypeConfiguration(configuration, type, formKey);
    ...
    getTypeConfiguration(configuration: any, type: string | null | undefined, formKey: string) {
