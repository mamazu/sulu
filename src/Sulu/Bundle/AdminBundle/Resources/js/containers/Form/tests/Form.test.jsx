...
test('Should render form using renderer', () => {
    ...
    const store = new ResourceFormStore_1.default(new ResourceStore_1.default('snippet', '1'), 'snippet');
    const form = (0, enzyme_1.render)(<Form_1.default onSubmit={submitSpy} store={store}/>);
...
test('Should call onSubmit callback', () => {
    ...
    const submitSpy = jest.fn();
    const store = new ResourceFormStore_1.default(new ResourceStore_1.default('snippet', '1'), 'snippet');
    metadataStore_1.default.getSchema.mockReturnValue({});
    const form = (0, enzyme_1.mount)(<Form_1.default onError={errorSpy} onSubmit={submitSpy} store={store}/>);
...
test.each([
    ...
    const resourceStore = new ResourceStore_1.default('snippet', '1');
    resourceStore.data = {
        block: [
            {
                text: 'Test',
    ...
    const store = new ResourceFormStore_1.default(resourceStore, 'snippet');
    const form = (0, enzyme_1.shallow)(<Form_1.default onSubmit={submitSpy} store={store}/>);
...
test('Should work when errors occurs but no onError callback is given', () => {
    ...
    store.errors = { error1: {} };
    metadataStore_1.default.getSchema.mockReturnValue({});
    const form = (0, enzyme_1.mount)(<Form_1.default onSubmit={submitSpy} store={store}/>);
    form.instance().submit();
    expect(submitSpy).not.toBeCalled();
});
test('Should validate form when a field has finished being edited', () => {
    const store = new ResourceFormStore_1.default(new ResourceStore_1.default('snippet', '1'), 'snippet');
    metadataStore_1.default.getSchema.mockReturnValue({});
    const form = (0, enzyme_1.mount)(<Form_1.default onSubmit={jest.fn()} store={store}/>);
...
test('Should pass data, onSuccess, router and schema to Renderer', () => {
    ...
    const form = (0, enzyme_1.shallow)(<Form_1.default onSubmit={jest.fn()} onSuccess={successSpy} router={router} store={store}/>);
...
test('Should pass showAllErrors flag to Renderer when form has been submitted', () => {
    ...
    form.find(Form_1.default).instance().submit();
    form.update();
...
test('Should show a GhostDialog if the current locale is not translated', () => {
    const resourceStore = new ResourceStore_1.default('snippet', '1', { locale: mobx_1.observable.box('de') });
    resourceStore.data.availableLocales = ['en'];
    const formStore = new ResourceFormStore_1.default(resourceStore, 'snippet');
    const form = (0, enzyme_1.mount)(<Form_1.default onSubmit={jest.fn()} store={formStore}/>);
...
test('Should not show a GhostDialog if the current locale is translated', () => {
    const resourceStore = new ResourceStore_1.default('snippet', '1', { locale: mobx_1.observable.box('en') });
    resourceStore.data.availableLocales = ['en'];
    const formStore = new ResourceFormStore_1.default(resourceStore, 'snippet');
    const form = (0, enzyme_1.mount)(<Form_1.default onSubmit={jest.fn()} store={formStore}/>);
...
test('Should not show a GhostDialog if the entity does not exist yet', () => {
    const resourceStore = new ResourceStore_1.default('snippet', undefined, { locale: mobx_1.observable.box('en') });
    resourceStore.data.availableLocales = ['en'];
    const formStore = new ResourceFormStore_1.default(resourceStore, 'snippet');
    const form = (0, enzyme_1.mount)(<Form_1.default onSubmit={jest.fn()} store={formStore}/>);
...
test('Should show a GhostDialog and copy the content if the confirm button is clicked', (resolve) => {
    metadataStore_1.default.getSchema.mockReturnValue(Promise.resolve(FORM));
    metadataStore_1.default.getJsonSchema.mockReturnValue(Promise.resolve({}));
    const resourceStore = new ResourceStore_1.default('snippet', '1', { locale: mobx_1.observable.box('de') });
    resourceStore.data.availableLocales = ['en'];
    resourceStore.loading = false;
    const formStore = new ResourceFormStore_1.default(resourceStore, 'snippet');
    const form = (0, enzyme_1.mount)(<Form_1.default onSubmit={jest.fn()} store={formStore}/>);
    metadataStore_1.default.getSchema().then(() => {
        setTimeout(() => {
...
test('Should show a GhostDialog and copy the content if the confirm button is clicked (with additional fields)', (resolve) => {
...
test('Should set the type of the formStore to selected value in MissingTypeDialog', () => {
    ...
    const form = (0, enzyme_1.mount)(<Form_1.default onMissingTypeCancel={onMissingTypeCancelSpy} onSubmit={jest.fn()} store={formStore}/>);
