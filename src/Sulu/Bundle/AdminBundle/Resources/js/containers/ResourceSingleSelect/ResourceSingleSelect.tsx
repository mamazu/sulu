...
@observer
class ResourceSingleSelect<T extends string | number> extends React.Component<Props<T>> {
    static defaultProps = {
        disabled: false,
        editable: false,
        requestParameters: {},
        ...
        return (
            <Fragment>
                <SingleSelect disabled={disabled} onChange={onChange} value={value}>
                    ...
                    {this.data.map((object, index) => (<SingleSelect.Option key={index} value={object[idProperty]}>
                        {object[displayProperty]}
                    </SingleSelect.Option> as Element<typeof SingleSelect.Option>))}
