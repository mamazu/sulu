...
test('Render data in EditLines', () => {
    ...
    const editOverlay = (0, enzyme_1.mount)(<EditOverlay_1.default displayProperty="title" idProperty="id" onClose={jest.fn()} open={true} resourceListStore={resourceListStore} title="Add something"/>);
...
test('Render data in EditLines with other properties', () => {
    ...
    const editOverlay = (0, enzyme_1.mount)(<EditOverlay_1.default displayProperty="position" idProperty="uuid" onClose={jest.fn()} open={true} resourceListStore={resourceListStore} title="Add something"/>);
    expect(editOverlay.find('header').render()).toMatchSnapshot();
    expect(editOverlay.find('article .overlay').render()).toMatchSnapshot();
});
test('Should only delete items from  ResourceStoreList if data is only deleted', () => {
    ...
    const editOverlay = (0, enzyme_1.mount)(<EditOverlay_1.default displayProperty="position" idProperty="uuid" onClose={closeSpy} open={true} resourceListStore={resourceListStore} title="Add something"/>);
