...
export default class BadgeStore {
    ...
    routerAttributesToRequest: any;
    tabViewRoute: Route;
    @observable
    value: string | null | undefined = null;
    routeChangeDisposer: () => Record<any, any>;
    ...
    @computed get isChildOrSameRoute() {
        let route: Route | null | undefined = this.router.route;
    ...
    load = debounce(() => {
        ...
        Requester.get(this.url).then((response: any) => {
            this.setData(response);
        });
    }, 3000, true);

    responseHook: HandleResponseHook = (response: Response, options?: any | null) => {
