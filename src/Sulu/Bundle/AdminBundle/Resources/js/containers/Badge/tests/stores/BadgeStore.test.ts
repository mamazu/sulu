...
const mockTabViewRoute = (Symbol() as any);
...
test('Should load data using the Requester', () => {
    SymfonyRouting.generate.mockImplementation((routeName: any, params: any) => {
        return routeName + '?' + Object.keys(params).map((key: any) => key + '=' + params[key]).join('&');
    });
...
test('Should load data if route changes', () => {
    ...
    router.route = ({property: 'value', parent: mockTabViewRoute} as any);
...
test('Should not load data if route changes to other parent', () => {
    ...
    router.route = ({property: 'value', parent: {other: true}} as any);
