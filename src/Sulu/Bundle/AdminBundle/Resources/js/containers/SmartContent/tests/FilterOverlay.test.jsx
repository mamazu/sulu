...
test('Do not display if open is set to false', () => {
    ...
    const filterOverlay = (0, enzyme_1.shallow)(<FilterOverlay_1.default categoryRootKey={undefined} dataSourceAdapter="table" dataSourceListKey="snippets" dataSourceResourceKey="snippets" defaultValue={defaultValue} onClose={jest.fn()} open={false} presentations={{}} sections={[]} smartContentStore={smartContentStore} sortings={[]} title="Test" types={[]}/>);
...
test('Pass rootKey for categories to options for category list', () => {
    ...
    const filterOverlay = (0, enzyme_1.shallow)(<FilterOverlay_1.default categoryRootKey="test1" dataSourceAdapter="table" dataSourceListKey="snippets" dataSourceResourceKey="snippets" defaultValue={defaultValue} onClose={jest.fn()} open={false} presentations={{}} sections={['categories']} smartContentStore={smartContentStore} sortings={[]} title="Test" types={[]}/>);
    expect(filterOverlay.find(MultiListOverlay_1.default).find('[resourceKey="categories"]').prop('options'))
...
test('Render with ListOverlays if smartContentStore is loaded', () => {
    ...
    const filterOverlay = (0, enzyme_1.shallow)(<FilterOverlay_1.default categoryRootKey={undefined} dataSourceAdapter="table" dataSourceListKey="snippets" dataSourceResourceKey="snippets" defaultValue={defaultValue} onClose={jest.fn()} open={true} presentations={{}} sections={['datasource', 'categories', 'tags', 'audienceTargeting', 'sorting', 'presentation', 'limit']} smartContentStore={smartContentStore} sortings={[]} title="Test" types={[]}/>);
    expect(filterOverlay.find(SingleListOverlay_1.default)).toHaveLength(1);
    expect(filterOverlay.find(MultiListOverlay_1.default)).toHaveLength(1);
});
test('Render without ListOverlays if smartContentStore is not loaded', () => {
    ...
    const filterOverlay = (0, enzyme_1.mount)(<FilterOverlay_1.default categoryRootKey={undefined} dataSourceAdapter={undefined} dataSourceListKey={undefined} dataSourceResourceKey={undefined} defaultValue={defaultValue} onClose={jest.fn()} open={true} presentations={{}} sections={['datasource', 'categories', 'tags', 'audienceTargeting', 'sorting', 'presentation', 'limit']} smartContentStore={smartContentStore} sortings={[]} title="Test" types={[]}/>);
    expect(filterOverlay.find(SingleListOverlay_1.default)).toHaveLength(0);
    expect(filterOverlay.find(MultiListOverlay_1.default)).toHaveLength(0);
});
...
test('Render with no fields', () => {
    ...
    const filterOverlay = (0, enzyme_1.mount)(<FilterOverlay_1.default categoryRootKey={undefined} dataSourceAdapter={undefined} dataSourceListKey={undefined} dataSourceResourceKey={undefined} defaultValue={defaultValue} onClose={jest.fn()} open={true} presentations={{}} sections={[]} smartContentStore={smartContentStore} sortings={[]} title="Test" types={[]}/>);
    expect(filterOverlay.find('Portal').at(1).render()).toMatchSnapshot();
});
test('Fill all fields using and update SmartContentStore on confirm', () => {
    ...
    const filterOverlay = (0, enzyme_1.mount)(<FilterOverlay_1.default categoryRootKey={undefined} dataSourceAdapter="table" dataSourceListKey="pages_list" dataSourceResourceKey="pages" defaultValue={defaultValue} onClose={closeSpy} open={true} presentations={{
            small: 'Small',
            large: 'Large',
        }} sections={[
            'datasource', 'categories', 'tags', 'audienceTargeting', 'sorting', 'presentation', 'limit', 'types',
        ]} smartContentStore={smartContentStore} sortings={[
    ...
    expect(filterOverlay.find(SingleListOverlay_1.default).find(pagesOptions).prop('open')).toEqual(true);
    filterOverlay.find(SingleListOverlay_1.default).find(pagesOptions).prop('onConfirm')({ id: 2, title: 'Test' });
    filterOverlay.update();
    expect(filterOverlay.find(SingleListOverlay_1.default).find(pagesOptions).prop('open')).toEqual(false);
    ...
    filterOverlay.update();
    expect(filterOverlay.find(MultiListOverlay_1.default).find({ resourceKey: 'categories' }).prop('open')).toEqual(true);
    filterOverlay.find(MultiListOverlay_1.default).find({ resourceKey: 'categories' }).prop('onConfirm')([
        { id: 1, name: 'Test1' },
        { id: 3, name: 'Test2' },
    ]);
    filterOverlay.update();
    expect(filterOverlay.find(MultiListOverlay_1.default).find({ resourceKey: 'categories' }).prop('open')).toEqual(false);
...
test('Prefill all fields with correct values', () => {
    ...
    const filterOverlay = (0, enzyme_1.mount)(<FilterOverlay_1.default categoryRootKey={undefined} dataSourceAdapter="table" dataSourceListKey="pages" dataSourceResourceKey="pages" defaultValue={defaultValue} onClose={jest.fn()} open={true} presentations={{
            small: 'Small',
            large: 'Large',
        }} sections={[
            'datasource', 'categories', 'tags', 'audienceTargeting', 'sorting', 'presentation', 'limit', 'types',
    ...
    expect(filterOverlay.find(SingleListOverlay_1.default).find({ resourceKey: 'pages' }).prop('preSelectedItem'))
        .toEqual({ id: 4, title: 'Homepage' });
    expect(filterOverlay.find('Toggler[children="sulu_admin.include_sub_elements"]').prop('checked')).toEqual(true);
    expect(filterOverlay.find('section').at(2).find('label[className="description"]').text())
        .toEqual('sulu_category.categories: Test1, Test3');
    expect(filterOverlay.find(MultiListOverlay_1.default).find(categoryOptions).prop('preSelectedItems'))
    ...
    expect(MultiSelectionStore_1.default).toBeCalledWith('tags', [1, 2], undefined, 'names');
    expect(filterOverlay.find('div[className="tags"]').find('SingleSelect').prop('value')).toEqual('and');
    expect(filterOverlay.find('div[className="types"]').find('MultiSelect').prop('values')).toEqual(['default', 'homepage']);
...
test('Reset all fields when reset action is clicked', () => {
    ...
    const filterOverlay = (0, enzyme_1.mount)(<FilterOverlay_1.default categoryRootKey={undefined} dataSourceAdapter="table" dataSourceListKey="pages" dataSourceResourceKey="pages" defaultValue={defaultValue} onClose={jest.fn()} open={true} presentations={{
            small: 'Small',
            large: 'Large',
        }} sections={['datasource', 'categories', 'tags', 'audienceTargeting', 'sorting', 'presentation', 'limit']} smartContentStore={smartContentStore} sortings={[
            { name: 'title', value: 'Title' },
            { name: 'created', value: 'Created' },
        ]} title="Test" types={[
            { name: 'default', value: 'default' },
