...
test('Load categories and datasource when constructed', () => {
    ...
    const dataSource = {id: 4} as const;
...
test('Reset dataSourceLoading if loading of dataSource fails', (done: any) => {
    const locale = observable.box('en');
...
test('Load items if FilterCriteria is given with categories', () => {
    const locale = observable.box('en');
    const params = {provider: {name: 'provider', value: 'content'}} as const;
    const filterCriteria = {
