...
export default class WritingAssistant extends React.Component<Props> {
    ...
    @action handleAddMessage = async(prompt: string, title?: string | null) => {
    ...
    @action optimizeText = async(prompt: string, title?: string | null) => {
        const {
            locale,
            url,
        } = this.props;
    ...
    handlePredefinedPromptButtonClick = (action: {
        name: string,
        prompt: string
    }) => {
    ...
    @computed
    get experts(): Array<ExpertType> {
        return (Object.values(this.props.configuration.experts) as Array<ExpertType>) || [];
    }
    ...
    @computed get expertsButton() {
        ...
        return {
            ...
            options: this.experts.map((expert: ExpertType): {
                id: string,
                name: string
            } => {
                return {
