...
jest.mock('../registries/linkTypeRegistry', () => ({
    ...
    getOverlay: jest.fn(),
    getOptions: jest.fn(),
    getTitle: jest.fn((key: any) => key.charAt(0).toUpperCase() + (key.slice(1))),
}));

test('Render Link container incl. loading a selected value', async(resolve: any) => {
...
test('Invalidate values on RemoveButton click', async(resolve: any) => {
    const changeSpy = jest.fn();
    const finishSpy = jest.fn();
