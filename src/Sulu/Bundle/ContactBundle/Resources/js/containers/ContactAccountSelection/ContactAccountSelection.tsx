...
class ContactAccountSelection extends React.Component<Props> {
    ...
    @observable
    openedOverlayType: string | null | undefined = undefined;
    ...
    @computed
    get loadedIds(): Array<string> {
        return toJS(this.store.items.map((item) => item.id));
    }

    @action handleAddButtonClick = (type?: string | null) => {
    ...
    @action handleConfirm(items: Array<any>, prefix: string) {
    ...
    @action handleContactConfirm = (contacts: Array<any>) => {
        this.handleConfirm(contacts, ContactAccountSelectionStore.contactPrefix);
    };

    @action handleAccountConfirm = (accounts: Array<any>) => {
    ...
    handleItemClick = (itemId: string, item?: any | null) => {
